...
@Service
public class ProformaDao extends InvoiceDao{
	public ProformaDao(){
		super("proformainvoices");
	}
	...
	public Page<ProformaInvoices> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal, InvoiceFilter filter){
		...
		query.with(pageable);
		long total = mongoTemplate.count(query, ProformaInvoices.class, COLLECTION_NAME);
		if (total == 0) {
			return new PageImpl<ProformaInvoices>(Collections.<ProformaInvoices> emptyList());
		}
		return new PageImpl<ProformaInvoices>(mongoTemplate.find(query, ProformaInvoices.class, COLLECTION_NAME), pageable, total);
	}
	
public Page<ProformaInvoices> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
	...
	public Page<ProformaInvoices> findByClientidAndInvtypeInAndFp(final String clientId, final List<String> invTypes, String fp, int start, int length, String searchVal){
		Criteria criteria = Criteria.where("clientid").is(clientId).and("invtype").in(invTypes).and("fp").is(fp);
		if(StringUtils.hasLength(searchVal)){					
			criteria = new Criteria().andOperator(criteria, getSearchValueCriteria(searchVal));
		}
	...
	private void addAllInvoicesQueryFirlds(Query query){
		...
		query.fields().include("convertedtoinv");
	}
