...
@Service
public class EstimatesDao extends InvoiceDao{
	public EstimatesDao(){
		super("estimates");
	}
	...
	public Page<Estimates> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal, InvoiceFilter filter){
		...
		query.with(pageable);
		long total = mongoTemplate.count(query, Estimates.class, COLLECTION_NAME);
		if (total == 0) {
			return new PageImpl<Estimates>(Collections.<Estimates> emptyList());
		}
		return new PageImpl<Estimates>(mongoTemplate.find(query, Estimates.class, COLLECTION_NAME), pageable, total);
	}
	
public Page<Estimates> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal, String fieldName, String order,InvoiceFilter filter){
	...
	public Page<Estimates> findByClientidAndInvtypeInAndFp(final String clientId, final List<String> invTypes, String fp, int start, int length, String searchVal){
		Criteria criteria = Criteria.where("clientid").is(clientId).and("invtype").in(invTypes).and("fp").is(fp);
		if(StringUtils.hasLength(searchVal)){					
			criteria = new Criteria().andOperator(criteria, getSearchValueCriteria(searchVal));
		}
