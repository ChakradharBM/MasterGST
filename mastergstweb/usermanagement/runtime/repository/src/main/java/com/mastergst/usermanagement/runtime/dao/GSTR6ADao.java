...
@Service
public class GSTR6ADao extends InvoiceDao {
	public GSTR6ADao(){
		super("gstr6a");
	}
	...
	public Page<? extends InvoiceParent> findByClientidAndMonthAndYear(String clientid, List<String> invTypes,String fp, int start, int length, String searchVal, String fieldName, String order,InvoiceFilter filter) {
		...
		if(start == 0 && length == 0) {
			return new PageImpl<GSTR6A>(mongoTemplate.find(query, GSTR6A.class, COLLECTION_NAME));
		}else {
		...
		if(NullUtil.isNotEmpty(fieldName) && NullUtil.isNotEmpty(order)){	
			...
			query.with(pageable);
			long total = mongoTemplate.count(query, GSTR6A.class, COLLECTION_NAME);
			if (total == 0) {
				return new PageImpl<GSTR6A>(Collections.<GSTR6A> emptyList());
			}
			return new PageImpl<GSTR6A>(mongoTemplate.find(query, GSTR6A.class, COLLECTION_NAME), pageable, total);
		}
	}
