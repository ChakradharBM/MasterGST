...
@Service
public class Gstr1SortDao extends InvoiceDao {
	public Gstr1SortDao(){
		super("gstr1");
	}
	...
	public Page<GSTR1> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
		Criteria criteria = Criteria.where("clientid").is(clientid).and("yrCd").is(yearCode);
		if(month > 0) {
			criteria.and("mthCd").is(month+"");
		}
	...
	public Page<GSTR1> findByClientidAndMonthAndYearQuarterly(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
		Criteria criteria = Criteria.where("clientid").is(clientid).and("yrCd").is(yearCode);
...
public Page<GSTR1> findByClientidAndMonthAndYearPrevMonthPendingInvoices(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
...
public TotalInvoiceAmount getTotalInvoicesAmountsForMonthPreviousMonthPendingInvoices(String clientId, int month, String yearCode, String searchVal, InvoiceFilter filter){
	...
	public Page<GSTR1> findByClientidAndFromtimeAndTotime(final String clientid, Date stDate, Date endDate, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
		//Criteria criteria = Criteria.where("clientid").is(clientid).andOperator(Criteria.where("dateofinvoice").gte(stDate),Criteria.where("dateofinvoice").lte(endDate));
		Criteria criteria = Criteria.where("clientid").is(clientid).and("dateofinvoice").gte(stDate).lte(endDate);
		applyFilterToCriteria(criteria, filter);
		if(StringUtils.hasLength(searchVal)){
	...
	public Page<GSTR1> findByClientidAndInvtypeInAndFp(final String clientId, final List<String> invTypes, String fp, int start, int length, String searchVal,String fieldName, String order){
		Criteria criteria = Criteria.where("clientid").is(clientId).and("invtype").in(invTypes).and("fp").is(fp);
		if(StringUtils.hasLength(searchVal)){					
			criteria = new Criteria().andOperator(criteria, getSearchValueCriteria(searchVal));
		}
	...
	public Page<GSTR1> findByClientidAndFp(final String clientId, String fp, int start, int length, String searchVal, String fieldName, String order,InvoiceFilter filter){
		Criteria criteria = Criteria.where("clientid").is(clientId).and("fp").is(fp);
		applyFilterToCriteria(criteria, filter);
		if(StringUtils.hasLength(searchVal)){
			criteria = new Criteria().andOperator(criteria, getSearchValueCriteria(searchVal));
	...
	public Page<GSTR1> findByClientidAndFpQuarterly(final String clientId, String fp,int month,int year, int start, int length, String searchVal, String fieldName, String order,InvoiceFilter filter){
		Criteria criteria = Criteria.where("clientid").is(clientId);
	...
	public Page<GSTR1> getMultimonthInvoices(String clientid, List<String> invTypes, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter) {
		Criteria criteria = Criteria.where("clientid").is(clientid)
	...
	public Page<? extends InvoiceParent> findByClientidInAndMonthAndYear(List<String> clientids, int month,String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter,String dwnldFromGSTN) {
	...
	public Page<? extends InvoiceParent> findByClientidInAndFromtimeAndTotime(List<String> clientids, Date stDate, Date endDate, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter, String dwnldFromGSTN) {
	...
	public List<TotalInvoiceAmount> getConsolidatedSummeryForGlobalYearMonthwise(List<String> clientids, String yearCode, boolean checkQuarterly,String dwnldFromGSTN,String gstrType,InvoiceFilter filter) {
		...
		GroupOperation groupOperation = Aggregation.group(checkQuarterly? "qrtCd": "mthCd")
				...
				.sum("totalExemptedAmount").as("totalExemptedAmount").count().as("totalTransactions");
		AggregationResults<TotalInvoiceAmount> results = mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, projectionOperation, groupOperation), COLLECTION_NAME, TotalInvoiceAmount.class);
		List<TotalInvoiceAmount> totalCanInvAmount = getCancelledConsolidatedSummeryForGlobalYearMonthwise(clientids,yearCode,checkQuarterly,dwnldFromGSTN,gstrType);
		List<TotalInvoiceAmount> totalInvAmount = results.getMappedResults();
		for(TotalInvoiceAmount gstr1CancelledInvoiceAmount : totalCanInvAmount){
			String cancode = gstr1CancelledInvoiceAmount.get_id();
			for(TotalInvoiceAmount gstr1InvoiceAmount : totalInvAmount){
...
public List<TotalInvoiceAmount> getConsolidatedSummeryForGlobalCustom(List<String> clientids, String returntype, Date stDate, Date endDate,String dwnldFromGSTN,String gstrType,InvoiceFilter filter) {
	...
	if(dwnldFromGSTN.equalsIgnoreCase("dwnldfromgstn")) {
		...
		GroupOperation groupOperation = Aggregation.group("mthCd")
				...
				.sum("totalExemptedAmount").as("totalExemptedAmount").count().as("totalTransactions");
		AggregationResults<TotalInvoiceAmount> results = mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, projectionOperation, groupOperation), COLLECTION_NAME, TotalInvoiceAmount.class);
		List<TotalInvoiceAmount> totalCanInvAmount = getCancelledConsolidatedSummeryForGlobalCustom(clientids,returntype,stDate,endDate,dwnldFromGSTN,gstrType);
		List<TotalInvoiceAmount> totalInvAmount = results.getMappedResults();
		for(TotalInvoiceAmount gstr1CancelledInvoiceAmount : totalCanInvAmount){
			String cancode = gstr1CancelledInvoiceAmount.get_id();
			for(TotalInvoiceAmount gstr1InvoiceAmount : totalInvAmount){
...
public List<TotalInvoiceAmount> getCancelledConsolidatedSummeryForGlobalYearMonthwise(List<String> clientids, String yearCode, boolean checkQuarterly,String dwnldFromGSTN,String gstrType) {
	Criteria criteria = null;
	if(gstrType.equalsIgnoreCase("gstrOrEinvoice")){
...
public List<TotalInvoiceAmount> getCancelledConsolidatedSummeryForGlobalCustom(List<String> clientids, String returntype, Date stDate, Date endDate,String dwnldFromGSTN,String gstrType) {
	Criteria criteria = null;
	if(gstrType.equalsIgnoreCase("gstrOrEinvoice")) {
...
public Page<GSTR1> findByClientidInAndMonthAndYear(List<String> clientids, List<String> invTypes, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, String booksorReturns,InvoiceFilter filter) {
	Criteria criteria = Criteria.where("clientid").in(clientids).and("yrCd").is(yearCode);
		//	.and("yrCd").is(yearCode).and("govtInvoiceStatus").is(MasterGSTConstants.SUCCESS);
...
public Page<GSTR1> findByClientidAndInvtypeInAndMonthAndYear(final String clientid,final List<String> invTypes, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
	Criteria criteria = Criteria.where("clientid").is(clientid).and("yrCd").is(yearCode);
	if(month > 0) {
		criteria.and("mthCd").is(month+"");
	}
...
public Page<GSTR1> findByClientidAndFromtimeAndTotimeEinvoice(final String clientid,final List<String> invTypes, Date stDate, Date endDate, int start, int length, String searchVal, String fieldName, String order,InvoiceFilter filter){
	//Criteria criteria = Criteria.where("clientid").is(clientid).andOperator(Criteria.where("dateofinvoice").gte(stDate),Criteria.where("dateofinvoice").lte(endDate));
	Criteria criteria = Criteria.where("clientid").is(clientid).and("dateofinvoice").gte(stDate).lte(endDate);
	if(filter.getInvoiceType() == null){
		criteria.and("invtype").in(invTypes);
...
public Page<GSTR1> findByClientidAndMonthAndYearPrevMonthPendingInvoices(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter, InvoiceFilter filter1){
	Criteria criteria = Criteria.where("clientid").is(clientid);
	String yrcd[] = yearCode.split("-");
	String yr = yrcd[0];
	int iyr = Integer.parseInt(yr);
