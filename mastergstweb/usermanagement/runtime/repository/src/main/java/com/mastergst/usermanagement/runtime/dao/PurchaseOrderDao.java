...
@Service
public class PurchaseOrderDao extends InvoiceDao{
	...
	public Page<PurchaseOrder> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal, InvoiceFilter filter){
		...
		query.with(pageable);
		long total = mongoTemplate.count(query, PurchaseOrder.class, COLLECTION_NAME);
		if (total == 0) {
			return new PageImpl<PurchaseOrder>(Collections.<PurchaseOrder> emptyList());
		}
		return new PageImpl<PurchaseOrder>(mongoTemplate.find(query, PurchaseOrder.class, COLLECTION_NAME), pageable, total);
	}
	
public Page<PurchaseOrder> findByClientidAndMonthAndYear(final String clientid, int month, String yearCode, int start, int length, String searchVal,String fieldName, String order, InvoiceFilter filter){
	...
	public Page<PurchaseOrder> findByClientidAndInvtypeInAndFp(final String clientId, final List<String> invTypes, String fp, int start, int length, String searchVal){
		Criteria criteria = Criteria.where("clientid").is(clientId).and("invtype").in(invTypes).and("fp").is(fp);
		if(StringUtils.hasLength(searchVal)){					
			criteria = new Criteria().andOperator(criteria, getSearchValueCriteria(searchVal));
		}
