...
@Controller
public class ImportWalterPackController {
	...
	@RequestMapping(value = "/importWalterpackInvoice/{id}/{name}/{usertype}/{clientid}/{returntype}/{month}/{year}", method = RequestMethod.POST)
				...
				if (status.isStatusOK()) {
					...
					for (String key : beans.keySet()) {
						if (isNotEmpty(sheetMap.get(key)) && sheetMap.get(key).size() > 0) {
				            ...
				            style.setFont(font);
						    Cell cell = datatypeSheet.getRow(0).createCell(datatypeSheet.getRow(1).getLastCellNum());
										...
										if (isNotEmpty(invoice.getB2b()) && isNotEmpty(invoice.getB2b().get(0).getCtin()) && !Pattern.matches(gstnFormat, invoice.getB2b().get(0).getCtin().trim().toUpperCase())) {
											...
											if(isNotEmpty(invoice.getB2b().get(0).getCtin())) {
												...
												if (returntype.equals(MasterGSTConstants.PURCHASE_REGISTER) || returntype.equals(MasterGSTConstants.GSTR2)) {
													...
													/*try {
														Date invDate = DateUtils.parseDate(invoice.getStrDate(),patterns);
														...
														try {
															double dblDate = Double.parseDouble(invoice.getStrDate());
