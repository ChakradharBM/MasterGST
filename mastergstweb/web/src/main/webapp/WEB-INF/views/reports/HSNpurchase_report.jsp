...
<title>MasterGST | HSN Purchase Report</title>
...
<link rel="stylesheet" href="${contextPath}/static/mastergst/css/reports/purchase_reports.css" media="all" />
...
.dropdown:hover .dropdown-content.reporthsnpurchase{display: block;}.arrow-up {width: 0; height: 0; border-left: 9px solid transparent;border-right: 9px solid transparent;border-bottom: 12px solid white; position: absolute;top: -8px;}.dropdown {position: relative;display: inline-block;}.dropdown-content.reporthsnpurchase{display: none;margin-top: 20px;position: absolute;background-color: white;min-width: 635px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.2);z-index: 1;color: black;padding: 12px 16px;text-decoration: none;margin-left: -13px; text-transform: capitalize;}.helpbtn.dropdown:hover .dropdown-content {display: block;}
</style>
...
var invoiceArray=new Object();
	...
	function getval(sel) {
		...
		if (sel == 'Custom') {$('.monthely-sp').css("display", "none");$('.yearly-sp').css("display", "none");$('.custom-sp').css("display", "inline-block");$('.dropdown-menu.ret-type').css("left", "16%");
		} else if (sel == 'Yearly') {$('.monthely-sp').css("display", "none");$('.yearly-sp').css("display", "inline-block");$('.custom-sp').css("display", "none");$('.dropdown-menu.ret-type').css("left", "19%");
		} else {$('.monthely-sp').css("display", "inline-block");$('.yearly-sp').css("display", "none");$('.custom-sp').css("display", "none");$('.dropdown-menu.ret-type').css("left", "19%");
		}
	};
	...
	if(abc == 'Monthly'){
		...
		var dwnldurl = "${contextPath}/dwnldxls/${id}/${client.id}/Purchase Register/"+mn+"/"+yr;
		$.ajax({
			url: "${contextPath}/getmonthlyinvs/${id}/${client.id}/Purchase Register/"+mn+"/"+yr,
			async: true,
			cache: false,
			dataType:"json",
			...
			success : function(invoiceList) {
				...
				if(invoiceList instanceof Array) {
					if(invoiceList.length > 0) {
						totalTaxableValue = 0;totalIGST = 0;totalCGST = 0;totalSGST = 0;totalCESS = 0;totalTax = 0;totalValue = 0;totalITC = 0;
						if ( $.fn.DataTable.isDataTable('#dbpurchaseshsnreport') ) {	$('#dbpurchaseshsnreport').DataTable().destroy();}
						$('#dbpurchaseshsnreport tbody').empty();
						var counts =0;	var hsnitems = {};var i=1;
						invoiceList.forEach(function(invoice){
							 if(invoice.invtype == 'B2B' || invoice.invtype == 'B2C' ||invoice.invtype == 'B2B Unregistered' || invoice.invtype == 'Exports' || invoice.invtype == 'Advance Adjusted Detail' || invoice.invtype == 'Advances' ||  invoice.invtype == 'Nil Supplies' || invoice.invtype == 'ITC Reversal' ||invoice.invtype == 'ISD' ||  invoice.invtype == 'Import Goods' || invoice.invtype == 'Import Services' || invoice.invtype == 'Credit/Debit Notes' || invoice.invtype == 'Credit/Debit Note for Unregistered Taxpayers') {
								if(invoice.items){
								...
								if(item.total == null){item.total = 0.00;}
										if(counts == 0){
									if(item.hsns != ""){
										hsnitems[item.hsns] = item;
									}
								}else{
								if(item.hsns in hsnitems){
									var hhssnn = item.hsns;
								...
								//content += '<tr>';content += '<td class="text-center"><span>'+i+'</span></td><td class="text-center"><span>'+item.hsns+'</span></td><td class="text-center"><span class="ind_formats">'+item.desc+'</span></td><td class="text-center"><span>'+item.quantity+'</span></td><td class="text-center"><span>'+formatNumber(item.total.toFixed(2))+'</span></td><td class="text-center"><span>'+formatNumber(item.taxablevalue.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(item.igstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(item.cgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(item.sgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(item.cessamount.toFixed(2))+'</span></td>';content += '</tr>';
								i++;
								counts++;
							});
							}
					...
					var objsize = Object.objsize(hsnitems);				
						for (var key in hsnitems) {
							content += '<tr>';content += '<td class="text-center"><span>'+j+'</span></td><td class="text-center"><span>'+hsnitems[key].hsns+'</span></td><td class="text-center"><span class="ind_formats">'+hsnitems[key].desc+'</span></td><td class="text-center"><span>'+hsnitems[key].quantity+'</span></td><td class="text-center"><span>'+formatNumber(hsnitems[key].total.toFixed(2))+'</span></td><td class="text-center"><span>'+formatNumber(hsnitems[key].taxablevalue.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].igstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].cgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].sgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].cessamount.toFixed(2))+'</span></td>';content += '</tr>';
							j++;
						}
						$('#monthlypurchaseshsn').html(content);
						salestable = $("#dbpurchaseshsnreport").DataTable({
							dom: 'lBfrtip', 		
							"paging": true,
							"searching": true,
			...
			var yearlydwnldurl = "${contextPath}/dwnldxlsyearly/${id}/${client.id}/Purchase Register/"+yrs;
		$.ajax({
			...
			success : function(invoiceList) {
				...
				if(invoiceList instanceof Array) {
					if(invoiceList.length > 0) {
						if ( $.fn.DataTable.isDataTable('#dbyearlypurchaseshsnreport') ) {$('#dbyearlypurchaseshsnreport').DataTable().destroy();}
						$('#dbyearlypurchaseshsnreport tbody').empty();
						var counts =0;	var hsnitems = []; var i=1;
						invoiceList.forEach(function(invoice){
							 if(invoice.invtype == 'B2B' || invoice.invtype == 'B2C' ||invoice.invtype == 'B2B Unregistered' || invoice.invtype == 'Exports' || invoice.invtype == 'Advance Adjusted Detail' || invoice.invtype == 'Advances' ||  invoice.invtype == 'Nil Supplies' || invoice.invtype == 'ITC Reversal' ||invoice.invtype == 'ISD' ||  invoice.invtype == 'Import Goods' || invoice.invtype == 'Import Services' || (invoice.invtype == 'Credit/Debit Notes' && invoice.cdn[0].nt[0].ntty == 'C') || (invoice.invtype == 'Credit/Debit Note for Unregistered Taxpayers' && invoice.cdnur[0].ntty == 'C')) {
								if(invoice.items){
									invoice.items.forEach(function(item) {
										if(item.hsn != '' && item.hsn != null){
										if(item.hsn.indexOf(':') > 0) {
					...
					var objsize = Object.objsize(hsnitems);				
						...
						$('#yearlypurchaseshsn').html(content);
						salestable = $("#dbyearlypurchaseshsnreport").DataTable({
							dom: 'lBfrtip',		
							"paging": true,
							"searching": true,
							"lengthMenu": [ [10, 25, 50, -1], [10, 25, 50, "All"] ],
			...
			$('.monthely1').css("display","none");$('.yearly1').css("display","none");$('.custom1').css("display","block");
	...
	$.ajax({
			url: "${contextPath}/getcustominvs/${id}/${client.id}/Purchase Register/"+fromtime+"/"+totime,
			async: true,
			cache: false,
			dataType:"json",
			contentType: 'application/json',
			...
			success : function(invoiceList) {
				...
				if(invoiceList instanceof Array) {
					if(invoiceList.length > 0) {
						if ( $.fn.DataTable.isDataTable('#custompurchaseshsnreport') ) {$('#custompurchaseshsnreport').DataTable().destroy();}
						$('#custompurchaseshsnreport tbody').empty();
						var counts =0;	var hsnitems = []; var i=1;
						invoiceList.forEach(function(invoice){
							 if(invoice.invtype == 'B2B' || invoice.invtype == 'B2C' ||invoice.invtype == 'B2B Unregistered' || invoice.invtype == 'Exports' || invoice.invtype == 'Advance Adjusted Detail' || invoice.invtype == 'Advances' ||  invoice.invtype == 'Nil Supplies' || invoice.invtype == 'ITC Reversal' ||invoice.invtype == 'ISD' ||  invoice.invtype == 'Import Goods' || invoice.invtype == 'Import Services' || (invoice.invtype == 'Credit/Debit Notes' && invoice.cdn[0].nt[0].ntty == 'C') || (invoice.invtype == 'Credit/Debit Note for Unregistered Taxpayers' && invoice.cdnur[0].ntty == 'C')) {
								if(invoice.items){
					...
					var objsize = Object.objsize(hsnitems);				
						...
						$('#custompurchaseshsn').html(content);
						salestable = $("#custompurchaseshsnreport").DataTable({
							dom: 'lBfrtip',	
							"paging": true,
							"searching": true,
							"lengthMenu": [ [10, 25, 50, -1], [10, 25, 50, "All"] ],
...
<body class="body-cls">
	...
	<div class="row">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active">HSN Purchase Report</li>
		</ol>
	...
	<div class="db-ca-wrap monthely1">
		<div class="container">
			...
			<h5>Monthly HSN Purchase Report  of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5><p>Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
			...
			<div class="dropdown-content reporthsnpurchase"> <span class="arrow-up"></span><span class="pl-2"> All the Purchase Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise Based On HSN</span></div>
			...
			<div class="dropdown chooseteam mr-0">
				<span class="dropdown-toggle yearly" data-toggle="dropdown" id="fillingoption" style="margin-right: 10px; display: inline-flex;">
					...
					<div class="typ-ret" style="border: 1px solid; z-index:2;border-radius: 2px; padding: 2px; background-color: white; padding-right: 14px; height: 27px; align-items: top; margin-left: 12px; min-width: 104px;">
						<span id="filing_option" class="filing_option"	style="vertical-align: top;">Monthly</span>
						<span class="input-group-addon add-on pull-right" style="display: inline-block; padding: 0px !important; border: none !important; background-color: unset !important; font-size: 1.5rem; position: relative; top: -7px; left: 8px;"><i
				...
				<span class="datetimetxt monthely-sp" style="display: block" id="monthely-sp"> 
				<span><label id="ret-period">Return Period:</label></span>
					<div class="datetimetxt datetime-wrap pull-right">
						...
						<a href="#" class="btn btn-greendark  pull-right" role="button"	style="padding: 4px 10px;" onClick="getdiv()">Generate</a>
					</div>
				</span> 
				<span style="display: none" class="yearly-sp"> 
					<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption1"	style="margin-right: 10px; display: inline-flex;">
						...
						<div class="typ-ret" style="border: 1px solid; z-index:2;border-radius: 2px; background-color: white; padding-right: 14px; height: 27px; align-items: top; min-width: 104px; max-width: 104px;">
		...
		<table id='dbpurchaseshsnreport' class="row-border dataTable meterialform" cellspacing="0" width="100%">
			<thead>
				<th class="text-center">S.NO</th><th class="text-center">HSN/SAC</th><th class="text-center">DESCRIPTION</th><th class="text-center dt-body-right">QUANTITY</th><th class="text-center dt-body-right">VALUE</th><th class="text-center dt-body-right">TAXABLE</th><th class="text-center dt-body-right">IGST</th><th class="text-center dt-body-right">CGST</th><th class="text-center dt-body-right">SGST</th><th class="text-center dt-body-right">CESS</th>
			</thead>
			<tbody id='monthlypurchaseshsn'></tbody>
		</table>
	</div>
		</div>
	</div>
	<div class="db-ca-wrap yearly1" style="display: none">
		<div class="container">
			<div class=" "></div>
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right" role="button" style="padding: 4px 25px;">Back</a>
			<h5>Yearly HSN Purchase Report  of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5>
			<p>Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
			...
			<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption" style="margin-right: 10px; display: inline-flex;">
				...
				<span style="display: inline-block" class="yearly-sp">
					<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption1"	style="margin-right: 10px; display: inline-flex;">
						...
						<div class="typ-ret" style="border: 1px solid;z-index:2; border-radius: 2px; padding: 2px; background-color: white; padding-right: 14px; height: 27px; align-items: top; min-width: 104px; max-width: 104px;">
			...
			<div class="customtable db-ca-view salestable">
		<table id='dbyearlypurchaseshsnreport' class="row-border dataTable meterialform" cellspacing="0" width="100%">
			<thead>
				<th class="text-center">S.NO</th><th class="text-center">HSN/SAC</th><th class="text-center">DESCRIPTION</th><th class="text-center dt-body-right">QUANTITY</th><th class="text-center dt-body-right">VALUE</th><th class="text-center dt-body-right">TAXABLE</th><th class="text-center dt-body-right">IGST</th><th class="text-center dt-body-right">CGST</th><th class="text-center dt-body-right">SGST</th><th class="text-center dt-body-right">CESS</th>
			</thead>
			<tbody id='yearlypurchaseshsn'></tbody>
		</table>
	</div>
		</div>
	</div>
	<div class="db-ca-wrap custom1" style="display: none">
		<div class="container">
			<div class=" "></div>
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right" role="button" style="padding: 4px 25px;">Back</a>
			<h5>Custom HSN Purchase Report  of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5>
			<p>HSN Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
			<div class="dropdown chooseteam  mr-0">
				<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption" style="margin-right: 10px; display: inline-flex;">
				...
				<span style="display: none margin-bottom:4px" class="yearly-sp"> 
					<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption1"	style="margin-right: 10px; display: inline-flex;">
						<label id="ret-period" style="margin-bottom: 3px;">Return Period:</label>
						<div class="typ-ret" style="border: 1px solid; z-index:2;border-radius: 2px; background-color: white; height: 27px; align-items: top; padding-right:14px; min-width: 104px;max-width: 104px;">
			...
			<div class="customtable db-ca-view salestable">
		<table id='custompurchaseshsnreport' class="row-border dataTable meterialform" cellspacing="0" width="100%">
			<thead>
				<th class="text-center">S.NO</th><th class="text-center">HSN/SAC</th><th class="text-center">DESCRIPTION</th><th class="text-center dt-body-right">QUANTITY</th><th class="text-center dt-body-right">VALUE</th><th class="text-center dt-body-right">TAXABLE</th><th class="text-center dt-body-right">IGST</th><th class="text-center dt-body-right">CGST</th><th class="text-center dt-body-right">SGST</th><th class="text-center dt-body-right">CESS</th>
			</thead>
			<tbody id='custompurchaseshsn'></tbody>
		</table>
	</div>
		</div>
	</div>
  ...
  <div class="modal-dialog modal-md modal-right" role="document">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <div class=" p-2 steptext-wrap"><span class="pl-2"> All the Purchase Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise Based On HSN</span> </div>
      </div>
	...
	<script>
		$(document).ready(function() {
			$( ".helpicon" ).hover(function() {$('.reporthsnpurchase ').show();
			}, function() {$('.reporthsnpurchase').hide();
			});
		...
		$.ajax({
			url: "${contextPath}/getmonthlyinvs/${id}/${client.id}/Purchase Register/${month}/${year}",
			async: true,
			cache: false,
			dataType:"json",
			contentType: 'application/json',
			...
			var content='';
				if(invoiceList instanceof Array) {
					...
					var objsize = Object.objsize(hsnitems);				
						...
						$(".salestable div.toolbar").append('<a href="${contextPath}/dwnldxls/${id}/${client.id}/Purchase Register/${month}/${year}" id="monthlydwnldxls" class="btn btn-blue">Excel<i class="fa fa-download" aria-hidden="true" style="margin-left: 6px;"></i></a>');
						invoiceArray['GSTR1'] = invoiceList;
						$('#monthProcess').text('');
					}else{
						salestable = $("#dbpurchaseshsnreport").DataTable({
