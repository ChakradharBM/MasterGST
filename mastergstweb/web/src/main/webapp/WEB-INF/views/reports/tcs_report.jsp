...
<meta charset="utf-8" />
    ...
    <title>MasterGST | TCS Report</title>
...
<%@include file="/WEB-INF/views/includes/client_header.jsp"%>
	...
	<div class="db-ca-wrap">
		...
		<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right mb-3" role="button" style="padding: 4px 25px;">Back</a>
			<h4>
				<span>TCS Report of ${client.businessname}</span>
			</h4>
			<div class=""></div>
			...
			<div class="noramltable-row mt-5">
			            ...
			            <div class="noramltable-row-desc">
			                ...
			                <div class="normaltable-col hdr filsummary">Total TCS<div class="normaltable-col-txt" id="idTDS"></div></div>
			            </div>
			        </div>
			        </div>
			        </div>
			        <div class="customtable db-ca-view reportTable reportTable4 fixed-col-div" id="tcsSummaryTable" style="display:none;">
				    <div class ="row">
					...
					<table id='tds_accounting_datatable' class="row-border dataTable meterialform" cellspacing="0" width="100%;">
						<thead>
							<tr>
								...
								<th class="text-center">Date of Collection</th>
								...
								<th class="text-center">TCS Collected</th>
							</tr>
						</thead>
	...
	function generateData() {
		...
		var pUrl = '';
		if(abc == 'Monthly'){
			$('#tcsSummaryTable').css("display", "none");
			...
			pUrl = "${contextPath}/gettdsdetails/${client.id}/"+month[0]+"/"+month[1]+"?report=tcsreport";
		}else if (abc == 'Yearly') {
			$('.reports-monthly').css("display", "none");$('.reports-yearly').css("display", "block");$('.reports-custom').css("display", "none");
			$('#tcsSummaryTable').css("display", "block");
			var year=$('#yearlyoption').html().split("-");
			pUrl = '${contextPath}/gettdsdetails/${client.id}/0/'+year[0]+"?report=tcsreport";
			var psummaryUrl = '${contextPath}/getsummarytdsdetails/${client.id}/0/'+year[0]+"?report=tcsreport";
			loadReportsSummary(psummaryUrl,'tcs');
		}else{
			...
			pUrl = '${contextPath}/gettdscustomdetails/${client.id}/'+fromtime+'/'+totime+"?report=tcsreport";
			var psummaryUrl = '${contextPath}/getsummarycustomtdsdetails/${client.id}/'+fromtime+'/'+totime+"?report=tcsreport";
			loadReportsSummary(psummaryUrl,'tcs');
		}
		loadReportsInvTable(abc, '<c:out value="${client.id}"/>','tcs', pUrl);	
	}
	function getval(sel) {
	...
	$(document).ready(function(){
		...
		var pUrl = '${contextPath}/gettdsdetails/${client.id}/'+month+"/"+year+"?report=tcsreport";
		loadReportsInvTable('Monthly', '<c:out value="${client.id}"/>','tcs', pUrl);
	});
	</script>
</body>
</html>