...
<head>
<title>MasterGST | GSTR2 VS GSTR2A</title>
...
<%@include file="/WEB-INF/views/includes/client_header.jsp" %>
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">GSTR2A - VS - GSTR2 Report</li>
					</ol>
  ...
  <div class="db-ca-wrap yearly1">
		<div class="container">
			...
			<h4>Invoice Wise GSTR2A vs Purchase Register Report</h4><p>This Report gives you the comparision between GSTR2-VS-GSTR2A Customer wise</p>
		 ...
		 <b>Comparision</b><span class="colon" style="margin-left: 3px;">:</span><span class="pl-2" style="display: block;">Differences between PurchaseRegister and GSTR2A (Purchase Data - (minus) GSTR2A Data)</span>
			</div>
		...
		<div class="customtable gstr2vs2atable">
                <table id="invoicetable" class="display row-border dataTable meterialform" cellspacing="0" width="100%" style="font-size:12px;">
				  ...
				  <tr>
                      ...
                      <th colspan="6" class="text-center diffhead">Differences (Purchase Register-GSTR2A)</th>
                    </tr>
                    <tr>
		...
		<div class="container">
			<div class=" "></div>
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right" role="button" style="padding: 4px 25px;">Back</a>
			<h4>Invoice Wise GSTR2A vs Purchase Register Report</h4><p>This Report gives you the comparision between GSTR2A vs Purchase Register Invoice Wise </p>
...
var gstnnumber='<c:out value="${client.gstnnumber}"/>';
		...
		function yearlyAjaxFuntion(year){
			var dwnldUrl ="${contextPath}/get2a_vs_2Yearly_dwnldExcel_invoice/${id}/${client.id}/"+year;
			$.ajax({
				url: "${contextPath}/get2a_vs_2Yearly_invoice/${id}/${client.id}/"+year,
				async: true,
				cache: false,
				dataType:"json",
				...
				success : function(invoice) {
					...
					if(invoice.length>0){
						...
						for(var i =0;i < invoice.length;i++){
							...
							content += '<tr>';
							content += '<td style="font-size:12px;" align="right" class="sdata">'+formatInvoiceDate(invoice[i].invoicedate)+'</td><td style="font-size:12px;" align="center" class="sdata">'+gstin+'</td><td style="font-size:12px;" align="center" class="sdata">'+fullname+'</td><td style="font-size:12px;" align="right" class="adata">'+invoice[i].invoiceno+'</td><td style="font-size:12px;" align="right" class="adata">'+formatNumber(invoice[i].gstr2AInvoiceValue.toFixed(2))+'</td><td style="font-size:12px;" align="right" class="adata">'+formatNumber(invoice[i].gstr2ATaxValue.toFixed(2))+'</td><td align="right" style="font-size:12px;" class="adata">'+formatNumber(invoice[i].gstr2AIGSTValue.toFixed(2))+'</td><td class="text-right adata" align="right" style="font-size:12px;">'+formatNumber(invoice[i].gstr2ACGSTValue.toFixed(2))+'</td><td class="text-right adata" align="right" style="font-size:12px;">'+formatNumber(invoice[i].gstr2ASGSTValue.toFixed(2))+'</td><td style="font-size:12px;" align="right" class="pdata"><div align="right">'+formatNumber(invoice[i].gstr2InvoiceValue.toFixed(2))+'</div></td><td class="text-right pdata">'+formatNumber(invoice[i].gstr2TaxValue.toFixed(2))+'</td><td class="fnt pdata"><div align="right">'+formatNumber(invoice[i].gstr2IGSTValue.toFixed(2))+'</div></td><td class="fnt pdata"><div align="right">'+formatNumber(invoice[i].gstr2CGSTValue.toFixed(2))+'</div></td><td class="fnt pdata"><div align="right">'+formatNumber(invoice[i].gstr2SGSTValue.toFixed(2))+'</div></td>';
						...
						$(".customtable div.toolbar").append('<a href="'+dwnldUrl+'" id="yearlydwnldxls" class="btn btn-blue permissionExcel_Download_In_Reports-Invoice_Wise_GSTR2_Vs_GSTR2A">Excel<i class="fa fa-download" aria-hidden="true" style="margin-left: 6px;"></i></a>');
					}else{
						$("#invoicetable").DataTable({
							"dom": '<"toolbar"f>lrtip<"clear">', 		
							"paging": true,
		...
		function customAjaxFuntion(fromtime,totime){
			var dwnldUrl="${contextPath}/get2a_vs_2Custom_dwnldExcel_invoice/${id}/${client.id}/"+fromtime+"/"+totime;
			$.ajax({
				url: "${contextPath}/get2a_vs_2Custom_invoice/${id}/${client.id}/"+fromtime+"/"+totime,
				async: true,
				cache: false,
				dataType:"json",
				contentType: 'application/json',
				...
				success : function(invoice) {
					...
					if(invoice.length>0){
						...
						$(".customtable div.toolbar").append('<a href="'+dwnldUrl+'" id="customdwnldxls" class="btn btn-blue permissionExcel_Download_In_Reports-Invoice_Wise_GSTR2_Vs_GSTR2A">Excel<i class="fa fa-download" aria-hidden="true" style="margin-left: 6px;"></i></a>');
					}else{
						$("#customInvoicetable").DataTable({
							"dom": '<"toolbar"f>lrtip<"clear">', 		
							"paging": true,
	...
	$.ajax({
		...
		success : function(invoice) {
			...
			if(invoice.length>0){
				...
				$(".customtable div.toolbar").append('<a href="'+dwnldUrl+'" id="monthlydwnldxls" class="btn btn-blue permissionExcel_Download_In_Reports-Invoice_Wise_GSTR2_Vs_GSTR2A">Excel<i class="fa fa-download" aria-hidden="true" style="margin-left: 6px;"></i></a>');
			}else{
				$("#invoicetable").DataTable({
					"dom": '<"toolbar"f>lrtip<"clear">', 		
					"paging": true,
