...
<title>MasterGST | GSTR1 Report</title>
...
function checkZero(data){
	...
	$(document).ready(function(){
		...
		gstr1_downloadurls(year);
	});
	function gstr1_ajaxfunction(year){
		var months = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
		var currentMonth=new Date().getMonth()+1;
		var curryear = new Date().getFullYear();
		$.ajax({
			url: "${contextPath}/gstr1status/${id}/${client.id}/"+year,
	...
	function gstr1_downloadurls(year){
		var mnths=['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
			...
			if(currentMonths <= 3 && year == currentyear-1 && year <= currentyear){
				...
				for(var i=1;i<=12;i++){
					...
					if(currentMonth == 1 || currentMonth == 2 || currentMonth == 3){
						if(i >= 4){
							$('.dwnldlink'+i).html('Download').attr('onclick','downloadgstr1data(\''+i+'\',\''+year+'\')');
							if(i>=10){
							...
							if(i<=currentMonth){
								$('.dwnldlink'+i).html('Download').attr('onclick','downloadgstr1data(\''+i+'\',\''+nxtyr+'\')');
	...
	function downloadgstr1data(mnth,year){
		otpandsubscriptionverification="";
		var months = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
		...
		if(otpandsubscriptionverification == 'OTP_VERIFIED'){
			...
			$.ajax({
				url: "${contextPath}/downloadsyncgstr1data/${id}/${client.id}/"+mnth+"/"+year,
				contentType: 'application/json',
				success : function(data) {
					if(data == "" || data == null){
						if(mnth < 10){
						...
						successNotification('No GSTR1 invoices found from GSTN for '+(mnths[mnth-1]+"-"+year));
						...
						successNotification('GSTR1 Invoices Downloaded from GSTN Successfully for '+(mnths[mnth-1]+"-"+year));
						$('.gstr3b_info div.spin-loader').remove();
						$('.gstr3b_info ul').css('opacity','1');
					}
...
<div class="breadcrumbwrap" >
	...
	<div class="row">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active">GSTR1 Report</li>
		</ol>
	...
	<div class="db-ca-wrap yearly1">
		<div class="container">
			...
			<h2>Yearly GSTR1 Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h2><p>Yearly GSTR1 Report gives you a summary of your annual sales.</p>
			<p style="color:red" class="dwnld-noticedinfo">We have noticed some of the months GSTR1 is not downloaded, please download to see full data set for this financial year.</p>
				...
				<div class="normaltable meterialform" id="updatefilter_summary_reports" style="display:none;">
					<div class="filter">
						<div class="noramltable-row">
							<div class="noramltable-row-hdr">Filter</div><div class="noramltable-row-desc"><div class="sfilter"><span id="divFiltersGSTR1"></span><span class="btn-remove-tag" onClick="clearInvFiltersReports('GSTR1','year')">Clear All<span data-role="remove"></span></span>
							</div>
							</div>
						</div>
					</div>
					<div class="noramltable-row">
						...
						<div class="noramltable-row">
							<div class="noramltable-row-hdr">Filter Summary</div>
								...
								<div class="normaltable-col hdr">Total Exempted<div class="normaltable-col-txt" id="idExemptedVal${varRetTypeCode}"></div></div>
	...
	<script type="text/javascript">
		$(function() {
			...
			//$('#dwnldxls').attr('href','${contextPath}/dwnldReportsFinancialSummaryxls/${id}/${client.id}/${varRetType}?reporttype=Multimonth-Reports&year='+year[0].trim()+'&fromdate=null&todate=null');	
		});
		function getdiv(){
			clearInvFiltersReportss('GSTR1');
			var year =$('#yearlyoption1').text().split("-");
			gstr1_downloadurls(parseInt(year[0]));
			loadReportsUsersByClient('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', loadReportsUsersInDropDown);
