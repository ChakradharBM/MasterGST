...
<title>MasterGST |Tax Slab wise Purchase Report</title>
...
<style>
.salestable{position:unset}
.dt-buttons :hover{color:white}
button.dt-button.buttons-excel.buttons-html5{width: auto;}
.dt-buttons{left: 59%;}
.dropdown:hover .dropdown-content.reporttaxpurchase{display: block;}.arrow-up {width: 0; height: 0; border-left: 9px solid transparent;border-right: 9px solid transparent;border-bottom: 12px solid white; position: absolute;top: -8px;}.dropdown {position: relative;display: inline-block;}.dropdown-content.reporttaxpurchase{display: none;margin-top: 20px;position: absolute;background-color: white;min-width: 660px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.2);z-index: 1;color: black;padding: 12px 16px;text-decoration: none;margin-left: -13px; text-transform: capitalize;}.helpbtn.dropdown:hover .dropdown-content {display: block;}
</style>
...
var invoiceArray=new Object();
	...
	if(abc == 'Monthly'){
		...
		$.ajax({
			url: "${contextPath}/getmonthlyinvs/${id}/${client.id}/Purchase Register/"+mn+"/"+yrs,
			async: true,
			cache: false,
			dataType:"json",
			contentType: 'application/json',
	...
	var customdwnldurl ="${contextPath}/dwnldxlscustom/${id}/${client.id}/Purchase Register/"+fromtime+"/"+totime;
	$.ajax({
...
<body class="body-cls">
	...
	<div class="row">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active">Tax Slab Wise Purchase Report</li>
		</ol>
	...
	<div class="db-ca-wrap monthely1">
		<div class="container">
			...
			<h5>Monthly Tax Slab Wise Purchase Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5><p>Tax Slab Wise Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
			...
			<div class="dropdown-content reporttaxpurchase"> <span class="arrow-up"></span><span class="pl-2"> All the Purchase Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise Based On Tax Rate</span></div>
	...
	<div class="db-ca-wrap yearly1" style="display: none">
		<div class="container">
			<div class=" "></div>
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right" role="button" style="padding: 4px 25px;">Back</a>
			<h5>Yearly Tax Slab Wise Purchase Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5>
			<p>Tax Slab Wise Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
	...
	<div class="db-ca-wrap custom1" style="display: none">
		<div class="container">
			<div class=" "></div>
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right" role="button" style="padding: 4px 25px;">Back</a>
			<h5>Tax Slab Wise Purchase Report  of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5>
			<p>Tax Slab Wise Purchase Report gives you a summary of your monthly, quarterly and annual purchases.</p>
			<div class="dropdown chooseteam  mr-0">
				<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="fillingoption" style="margin-right: 10px; display: inline-flex;">
  ...
  <div class="modal-dialog modal-md modal-right" role="document">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <div class=" p-2 steptext-wrap"><span class="pl-2"> All the Purchase Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise Based On Tax Rate</span> </div>
      </div>
	...
	<script>
		$(document).ready(function() {
			$( ".helpicon" ).hover(function() {$('.reporttaxpurchase ').show();
			}, function() {$('.reporttaxpurchase').hide();
			});
		...
		$.ajax({
			...
			var content='';
				if(invoiceList instanceof Array) {
					...
					var objsize = Object.objsize(hsnitems);				
						for (var key in hsnitems) {
							content += '<tr>';content += '<td class="text-center"><span>'+j+'</span></td><td class="text-center"><span>'+hsnitems[key].rate+'</span></td><td class="text-center"><span>'+hsnitems[key].quantity+'</span></td><td class="text-center"><span>'+formatNumber(hsnitems[key].rateperitem.toFixed(2))+'</span></td><td class="text-center"><span>'+formatNumber(hsnitems[key].taxablevalue.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].igstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].cgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].sgstamount.toFixed(2))+'</span></td><td class="text-center"><span class="ind_formats">'+formatNumber(hsnitems[key].cessamount.toFixed(2))+'</span></td>';content += '</tr>';
							j++;
						}
						$('#monthlypurchaseshsn').html(content);
						salestable = $("#dbpurchaseshsnreport").DataTable({
