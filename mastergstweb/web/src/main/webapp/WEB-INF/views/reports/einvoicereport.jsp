...
<head>
    ...
    <title>MasterGST | E-invoice Report</title>
...
<body class="body-cls">
	     	...
	     	<div class="row">
	        	<div class="col-md-12 col-sm-12">
	            	<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">E-invoice Report</li>
	                </ol>
	...
	<div class="db-ca-wrap">
		<div class="container">
			...
			<h2>
				<span class="reports-monthly">Monthly E-invoice Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-yearly" style="display: none;">Yearly E-invoice Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-custom" style="display: none;">Custom E-invoice Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
			</h2>
			<p>
				<span class="reports-monthly">Monthly E-invoice Report gives you a summary of your monthly generated IRN's.</span>
				<span class="reports-yearly" style="display: none;">Monthly E-invoice Report gives you a summary of your generated IRN's.</span>
				<span class="reports-custom" style="display: none;">Monthly E-invoice Report gives you a summary of your generated IRN's.</span>
			</p>
			        ...
			        <div class="noramltable-row">
			            ...
			            <div class="noramltable-row-desc">
			                ...
			                <select id="multiselect${varRetTypeCode}2" class="multiselect-ui form-control" multiple="multiple">
			                    <option value="<%=MasterGSTConstants.ADVANCES%>">Advance Tax</option><option value="<%=MasterGSTConstants.ATPAID%>">Advance Adjusted</option><option value="B2B">B2B Invoices</option><option value="B2C">B2CS (Small) Invoices</option><option value="B2CL">B2CL (Large)</option><option value="<%=MasterGSTConstants.CREDIT_DEBIT_NOTES%>"><%=MasterGSTConstants.CREDIT_DEBIT_NOTES%></option><option value="<%=MasterGSTConstants.CDNUR%>">Credit/Debit Note for Unregistered</option><option value="<%=MasterGSTConstants.EXPORTS%>"><%=MasterGSTConstants.EXPORTS%></option><option value="<%=MasterGSTConstants.NIL%>">Nil Rated / Exempted / Non-GST</option>
	...
	<script src="${contextPath}/static/mastergst/js/reports/einvoice_report.js" type="text/javascript"></script>
	<script type="text/javascript">
		$(function(){
			...
			loadEinvReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', '${month}', '${year}', '${usertype}', '${fullname}', booksOrReturns,'Monthly');
		...
		function generateData() {
			...
			if(abc == 'Monthly'){
				...
				loadEinvReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', mn, yr, '${usertype}', '${fullname}', booksOrReturns, 'Monthly');
				...
				loadEinvReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, year[1], '${usertype}', '${fullname}', booksOrReturns, 'Yearly');
				...
				var type ='custom';
				//loadReportsUsersByClient('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', loadReportsUsersInDropDown);
				loadReportsInvoiceSupport('${client.id}', '${varRetType}', '${varRetTypeCode}', fromtime, totime, 'custom', loadReportsCustomersInDropdown);
				loadEinvReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', fromtime, totime, '${usertype}', '${fullname}', booksOrReturns, 'custom');
