...
$(document).ready(function(){
	...
	gstr2a_downloadurls(year);
});
function gstr2a_ajaxfunction(year){
	var months = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
	var currentMonth=new Date().getMonth()+1;
	var curryear = new Date().getFullYear();
	$.ajax({
		url: "${contextPath}/gstr2bstatus/${id}/${client.id}/"+year,
...
function excelreportgstr2a(mnth,year){
	otpandsubscriptionverification="";
	var months = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
		...
		$.ajax({
			...
			success : function(data) {
				if(data == "" || data == null){
					...
					successNotification('No GSTR2A invoices found from GSTN for '+(mnths[mnth-1]+"-"+year));
					...
					successNotification('GSTR2A Invoices Downloaded from GSTN Successfully for '+(mnths[mnth-1]+"-"+year));
					$('.gstr3b_info div.spin-loader').remove();
					$('.gstr3b_info ul').css('opacity','1');
				}
...
function gstr2a_downloadurls(year){
	var mnths=['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'];
			...
			if(currentMonths <= 3 && year == currentyear-1 && year <= currentyear){
				...
				for(var i=1;i<=12;i++){
					...
					if(currentMonth == 1 || currentMonth == 2 || currentMonth == 3){
						if(i >= 4){
							$('.dwnldlink'+i).html('Download').attr('onclick','excelreportgstr2a(\''+i+'\',\''+year+'\')');
							if(i>=10){
							...
							if(i<=currentMonth){
								$('.dwnldlink'+i).html('Download').attr('onclick','excelreportgstr2a(\''+i+'\',\''+nxtyr+'\')');
...
<jsp:include page="/WEB-INF/views/client/otpverification.jsp"/>
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">MultiMonth GSTR2A Report</li>
					</ol>
  ...
  <div class="db-ca-wrap yearly1">
		<div class="container">
			...
			<h2>Yearly GSTR2A Report</h2><p>GSTR2A Report gives you a summary of your quarterly	and annual purchases.</p>
			<p style="color:red" class="dwnld-noticedinfo">We have noticed some of the months GSTR2A is not downloaded, please download to see full data set for this financial year.</p>
			...
			<div class="dropdown-content reportSummary"> <span class="arrow-up"></span><span class="pl-2"> All Filed Invoices Downloaded From GSTIN Yearly Wise</span></div>
				...
				<div class="filter">
					<div class="noramltable-row">
						...
						<div class="sfilter">
							...
							<span class="btn-remove-tag" onClick="clearInvFiltersReportss('GSTR2A')">Clear All<span data-role="remove"></span></span>
						</div>
						</div>
					</div>
				</div>
	...
	<div class="customtable db-ca-view reportTable reportTable4 fixed-col-div" id="${varRetTypeCode}SummaryTable">
		<div class ="row">
			<div class="col-sm-9 pr-0"><h4><span class="reports-yearly" style="display: none;">Yearly Summary of <c:out value="${client.businessname}"/></span><span class="reports-custom" style="display: none;">Custom Summary of <c:out value="${client.businessname}"/></span></h4></div>
			<div class="col-sm-3">
	...
	<div class="customtable db-ca-view salestable reportsdbTable_${varRetTypeCode}">
		<table id='reports_dataTableGSTR2A' class="row-border dataTable meterialform" cellspacing="0" width="100%">
			<thead>
				<tr>
  ...
  <div class="modal-dialog modal-md modal-right" role="document">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <div class=" p-2 steptext-wrap"><span class="pl-2"> All Filed Invoices Downloaded From GSTIN Yearly Wise</span> </div>
      </div>
...
<script src="${contextPath}/static/mastergst/js/reports/multimonth_report.js" type="text/javascript"></script>
<script type="text/javascript">
	$(function() {
		...
		loadReportsInvoiceSupport('${client.id}', '${varRetType}', '${varRetTypeCode}', 0, yr[1], loadReportsCustomersInDropdown);
		loadReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, yr[1], yr[0]);
		loadReportsSummary('${id}', '${client.id}', '${varRetType}', 0, yr[1]);
		...
		invsDownloads('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, yr[1], yr[0]);
	});
	function getdiv() {
		var year =$('.yearlyoption').text().split("-");
		$('.dwnld-noticedinfo').removeClass('d-none');
		clearInvFiltersReportss('GSTR2A');
		gstr2a_downloadurls(parseInt(year[0]));
		loadReportsUsersByClient('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', loadReportsUsersInDropDown);
		loadReportsInvoiceSupport('${client.id}', '${varRetType}', '${varRetTypeCode}', 0, year[1], loadReportsCustomersInDropdown);
		loadReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, year[1], year[0]);
		loadReportsSummary('${id}', '${client.id}', '${varRetType}', 0, year[1]);
		initiateCallBacksForMultiSelectReports('${varRetType}','${varRetTypeCode}');
		initializeRemoveAppliedFiltersReports('${varRetType}','${varRetTypeCode}');
		invsDownloads('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, year[1], year[0]);
	}
