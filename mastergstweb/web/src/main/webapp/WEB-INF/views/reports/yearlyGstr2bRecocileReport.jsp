...
<head>
<title>MasterGST | Purchase Register & GTSR2B yearly Reconciliation Report</title>
...
<script src="${contextPath}/static/mastergst/js/reports/yearlyGstr2bReconsileReport.js" type="text/javascript"></script>
<style>
...
var selG2bMatchArray = new Array();
...
<jsp:include page="/WEB-INF/views/client/otpverification.jsp"/>
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">Purchase Register & GTSR2B yearly Reconciliation Report</li>
					</ol>
  ...
  <div class="db-ca-wrap yearly1">
		<div class="container">
			...
			<h2>Purchase Register & GTSR2B yearly Reconciliation Report</h2><p>Financial year period invoices of Purchase Register.<br>Financial year starting to till now records of GSTR2B invoices.</p>
			...
			<div class="dropdown-content reportSummary" style="right:unset"> <span class="arrow-up"></span><span class="pl-2">Financial year period invoices of Purchase Register.</span><br><span class="pl-2">Financial year starting to till now records of GSTR2B invoices.</span></div>
			...
			<span class="summary_retperiod">
				<a href="#" data-toggle="modal" data-target="#reconcile2bmodal" style="color: #404144;text-decoration: underline;">Reconcile Summary</a></span> of FY <span id="retperiod"></span><span style="font-size:13px; color:#0dce2f; font-weight:bold;margin-left:3px"> <c:if test="${empty client.gstr2bReconcileDate }"><span style="color:red;font-size:13px">You Haven't Reconciled Yet,</span></c:if><c:if test="${not empty client.gstr2bReconcileDate }">Last Reconciled on: ${client.gstr2bReconcileDate}</c:if> </span> 
				<c:if test="${reconcileCounts eq null}">
					<a href="#" onClick="reconcileYearlyGstr2b()" class="btn btn-blue 2breconbtn" style="color: #404144;;margin-left:10px;font-size: 12px;float: right; position: absolute; padding: 2px 6px 0px;">Reconcile Now</a>
				</c:if>
				<c:if test="${not empty reconcileCounts}">
				<!-- style="margin-left: 116px;" -->
					<a href="#" onClick="reconcileYearlyGstr2b()" class="btn btn-blue 2breconbtn d-none" style="color: #404144;;margin-left:10px;font-size: 12px;float: right; position: absolute; padding: 2px 6px 0px;">Reconcile Now</a>
					<span style="font-size: 14px;" class ="rccn">Reconciliation is in Process, <img src="${contextPath}/static/mastergst/images/master/gears.gif" width="40px" style="margin-left:6px;margin-right:6px;"/> <a href="#" onclick="verify2bReconsilation('${client.id}')">click here </a>for status</span>
				</c:if>
				<span style="font-size: 14px;" class="processverification d-none">Reconciliation is in Process, <c:out value="${reconcileCounts}"/>  <img src="${contextPath}/static/mastergst/images/master/gears.gif" width="40px" style="margin-left:6px;margin-right:6px;"/> <a href="#" onclick="verify2bReconsilation('${client.id}')">click here </a>for status</span>
	...
	<!-- <a href="#" class="btn btn-blue sendmessage disable" data-toggle="modal" data-target="#sendMessageModal">Send Message</a> --> <a href="#" class="btn btn-blue reconModal g2bMannualMatching disable" data-toggle="modal" data-target="#reconModal" onclick="g2bMannualMatchingInv('${client.id}','yearly')">Manual Match</a></h4>
    ...
    <div class="customtable db-ca-view tabtable2 reconciletablee">
					<div class="customtable db-ca-view" style="margin-top: 65px;">
						...
						<table id="gstr2bReconsileDbTable" class="row-border dataTable meterialform" cellspacing="0" width="100%">
							<thead>
								<tr>
									<th class="text-center"><div class="checkbox"> <label><input type="checkbox" id="checkMismatch" onClick="updateGstr2bMainMisMatchSelection(this)"/><i class="helper"></i></label></div> </th>
									<th> </th>
  ...
  <div class="modal-dialog modal-md modal-right" role="document">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <div class=" p-2 steptext-wrap"><span class="pl-2">Financial year period invoices of Purchase Register.</span><br><span class="pl-2">Financial year starting to till now records of GSTR2B invoices.</span> </div>
      </div>
...
$(function() {
	...
	loadYearlyGstr2bReconcileReportsUsersByClient('${id}', '${client.id}', loadYearlyGstr2bReconcileReportsUsersInDropDown);
	loadYearlyGstr2bReconcileReportsInvoiceSupport('${id}', '${client.id}', parseInt(yr[0].trim()), loadYearlyGstr2bReconcileReportsCustomersInDropdown);
	loadYearlyGstr2bReconcileReportsInvTable('${id}', '${client.id}', parseInt(yr[0].trim()));
	initiateCallBacksForMultiSelectGstr2bReconcile();
	initializeRemoveAppliedFiltersGstr2bReconcile();
	loadYearlyGstr2bReconcileReportsReconcileSummary('${id}', '${client.id}', parseInt(yr[0].trim()));
	var dwnldExcel = "${contextPath}/dwnldReconsilexls2b/${id}/${client.id}/GSTR2B/4/"+parseInt(yr[0].trim())+"?dwnldtype=invoicewise&isYearly=Yearly";
	var alldetailsdwnldExcel = "${contextPath}/dwnldReconsilexls2b/${id}/${client.id}/GSTR2B>/4/"+parseInt(yr[0].trim())+"?dwnldtype=alldetailswise&isYearly=Yearly";
...
function getdiv() {
	...
	$('#retperiod').text(fp);
	loadYearlyGstr2bReconcileReportsUsersByClient('${id}', '${client.id}', loadYearlyGstr2bReconcileReportsUsersInDropDown);
	loadYearlyGstr2bReconcileReportsInvoiceSupport('${id}', '${client.id}', yrs, loadYearlyGstr2bReconcileReportsCustomersInDropdown);
	loadYearlyGstr2bReconcileReportsInvTable('${id}', '${client.id}', yrs);
	initiateCallBacksForMultiSelectGstr2bReconcile();
	initializeRemoveAppliedFiltersGstr2bReconcile();
	loadYearlyGstr2bReconcileReportsReconcileSummary('${id}', '${client.id}', yrs);
	var dwnldExcel = "${contextPath}/dwnldReconsilexls2b/${id}/${client.id}/GSTR2B/4/"+yrs+"?dwnldtype=invoicewise&isYearly=Yearly";
	var alldetailsdwnldExcel = "${contextPath}/dwnldReconsilexls2b/${id}/${client.id}/GSTR2B>/4/"+yrs+"?dwnldtype=alldetailswise&isYearly=Yearly";
	$('#downloadExcelLink').attr('href',dwnldExcel);
	$('#downloadExcelLinkAlldetails').attr('href',alldetailsdwnldExcel);
}
function reconcileYearlyGstr2b(){
	var fp = $('.yearlyoption').text();var fpsplit = fp.split(' - ');var yrs = parseInt(fpsplit[0]);var yrs1 = parseInt(fpsplit[0])+1;
	...
	$.ajax({
		...
		success : function(response) {
			$('.2breconbtn').addClass("d-none");
			$('.processverification').removeClass("d-none");
			if(response == 'verified') {
				var urlStr = '${contextPath}/gstr2breconcileinv/${id}/${fullname}/${usertype}/${client.id}/GSTR2B/4/'+yrs1+'?isYearly=Yearly';
				$.ajax({
					url: urlStr,
					async: true,
					cache: false,
...
function execute2bQuery(id) {
	...
	function updateCall(id){
		timer = setTimeout(function(){execute2bQuery(id)}, 1000);
	}
</script>
</body>
</html>