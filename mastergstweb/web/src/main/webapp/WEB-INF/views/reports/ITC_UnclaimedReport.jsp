...
<title>MasterGST | ITC Unclaimed Report</title>
...
.dropdown:hover .dropdown-content.reportunclaim{display: block;}.arrow-up {width: 0; height: 0; border-left: 9px solid transparent;border-right: 9px solid transparent;border-bottom: 12px solid white; position: absolute;top: -8px;}.dropdown {position: relative;display: inline-block;}.dropdown-content.reportunclaim{display: none;margin-top: 20px;position: absolute;background-color: white;min-width: 570px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.2);z-index: 1;color: black;padding: 12px 16px;text-decoration: none;margin-left: -13px; text-transform: capitalize;}.helpbtn.dropdown:hover .dropdown-content {display: block;}
</style>
</head>
<body class="body-cls">
...
<div class="breadcrumbwrap" >
	<div class="container bread">
		<div class="row">
			...
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">ITC Unclaimed Report</li>
			</ol>
		<div class="retresp"></div></div></div>
	</div>
</div><div class="container" style="padding-top:130px;">
	...
	<span class="reports-monthly"><h5>Monthly ITC Unclaimed Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5><p>Monthly ITC Unclaimed Report gives you a summary of your monthly purchaes.</p></span>
	<span class="reports-yearly" style="display: none;"><h5>Yearly ITC Unclaimed Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5><p>Yearly ITC Unclaimed Report gives you a summary of your annual Purchases.</p></span>
	<span class="reports-custom" style="display: none;"><h5>Custom ITC Unclaimed Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h5><p>Custom ITC  Claimed  Report gives you a summary of your monthly, quarterly and annual Purchases.</p></span>
	...
	<div class="">
			...
			<div class="dropdown-content reportunclaim"> <span class="arrow-up"></span><span class="pl-2"> All the UnClaimed Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise</span></div>
			...
			<div class="customtable db-ca-view salestable reportsdbTable">
				<table id="dbItcInvoiceTable" class="row-border dataTable meterialform" cellspacing="0" width="100%">
					<thead>
						<tr>
							<th class="text-center">Type</th><th class="text-center">Invoice No</th><th class="text-center dt-body-right">Suppliers</th><th class="text-center dt-body-right">GSTIN</th><th class="text-center dt-body-right">Date</th><th class="text-center dt-body-right">Taxable Amt</th><th class="text-center dt-body-right">Total Tax</th><th class="text-center dt-body-right">Total Amt</th><th class="text-center dt-body-right">ITC Claimed</th>
						</tr>
					</thead>
	...
	<div class="modal-dialog modal-md modal-right" role="document">
	    <div class="modal-content">
			<div class="modal-body">
		        ...
		        <div class=" p-2 steptext-wrap"><span class="pl-2"> All the UnClaimed Invoices from your PurchaseRegister/Books Monthly, Yearly and Custom Wise</span> </div>
		    </div>
...
$(function(){
	...
	loadItcInvoices('${id}', '${clientid}', '${varRetType}', '${varRetTypeCode}', month, year, "itc_unclaimed","monthly");
});
function generateData() {
	...
	if(abc == 'Monthly'){
		...
		loadItcInvoices('${id}', '${clientid}', '${varRetType}', '${varRetTypeCode}', 0, year[0], "itc_unclaimed", "yearly");
	}else{
		$('#${varRetTypeCode}SummaryTable').css("display", "block");
		...
		loadItcInvoices('${id}', '${clientid}', '${varRetType}', '${varRetTypeCode}', fromtime, totime, "itc_unclaimed", "custom");
	}
}
</script>
</body>
