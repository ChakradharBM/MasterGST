...
<c:set var="varRetType" value="<%=MasterGSTConstants.EWAYBILL%>"/>
...
<head>
    ...
    <title>MasterGST | E-way Bill Report</title>
...
<body class="body-cls">
	     	...
	     	<div class="row">
	        	<div class="col-md-12 col-sm-12">
	            	<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">E-way Bill Report</li>
	                </ol>
	...
	<div class="db-ca-wrap">
		<div class="container">
			...
			<h4>
				<span class="reports-monthly">Monthly E-way Bill Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-yearly" style="display: none;">Yearly E-way Bill Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-custom" style="display: none;">Custom E-way Bill Report of <c:choose><c:when test='${fn:length(client.businessname) > 25}'>${fn:substring(client.businessname, 0, 25)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
			</h4>
			<p>
				<span class="reports-monthly">Monthly E-way Bill Report gives you a summary of your monthly generated E-way bills.</span>
				<span class="reports-yearly" style="display: none;">Yearly E-way Bill Report gives you a summary of your yearly generated E-way bills.</span>
				<span class="reports-custom" style="display: none;">Custom E-way Bill Report gives you a summary of your custom generated E-way bills.</span>
			</p>
			<div class="helpguide reporthelpguide dropdown helpicon" data-toggle="modal" data-target="#reporthelpGuideModal" style="display:flex;float:left;margin-top:0px;">
				Help To Read This Report<div class="dropdown-content reportSales"> <span class="arrow-up"></span><span class="pl-2"> All Generated E-way Bill Invoices from your Monthly, Yearly and Custom Wise</span></div>
			</div>
			...
			<div class="tab-pane" id="gtab1" role="tabpanel">
			    <div class="normaltable meterialform" id="updatefilter_summary_reports" style="display:none;">
			        <div class="filter">
			            <div class="noramltable-row">
			                ...
			                <div class="noramltable-row-desc">
			                    <div class="sfilter"><span id="divFiltersEWAYBILL"></span>
			                        <span class="btn-remove-tag" onClick="clearInvFiltersReports('EWAYBILL','')">Clear All<span data-role="remove"></span></span>
			                    </div>
			                </div>
			            </div>
			        </div>
			        <div class="noramltable-row">
			            ...
			            <div class="noramltable-row-desc">
			                <select id="multiselect${varRetTypeCode}1" class="multiselect-ui form-control" multiple="multiple"></select>
			        ...
			        <div class="noramltable-row">
			            ...
			            <div class="noramltable-row-desc">
			                <div class="normaltable-col hdr">Total Invoices<div class="normaltable-col-txt" id="idCountEWAYBILL"></div></div>
			                <div class="normaltable-col hdr">Total Taxable Value<div class="normaltable-col-txt" id="idTaxableValEWAYBILL"></div></div>
			                <div class="normaltable-col hdr">Total Exempted<div class="normaltable-col-txt" id="idExemptedValEWAYBILL"></div></div>
			                <div class="normaltable-col hdr">Total Tax Value<div class="normaltable-col-txt" id="idTaxValEWAYBILL"></div></div>
			                <div class="normaltable-col hdr">Total Amount<div class="normaltable-col-txt" id="idTotAmtValEWAYBILL"></div></div>
			                <div class="normaltable-col hdr filsummary">Total IGST<div class="normaltable-col-txt" id="idIGSTEWAYBILL"></div></div>
			                <div class="normaltable-col hdr filsummary">Total CGST<div class="normaltable-col-txt" id="idCGSTEWAYBILL"></div></div>
			                <div class="normaltable-col hdr filsummary">Total SGST<div class="normaltable-col-txt" id="idSGSTEWAYBILL"></div></div>
			                <div class="normaltable-col hdr filsummary">Total CESS<div class="normaltable-col-txt" id="idCESSEWAYBILL"></div></div>
			            </div>
			        </div>
			    </div>
				...
				<div class="customtable db-ca-view salestable reportsdbTableEWAYBILL">
					<table id='reports_dataTableEWAYBILL' class="row-border dataTable meterialform" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th>Eway Bill No</th><th>Type</th><th class="text-center">Document No</th><th class="text-center">Customers</th><th class="text-center">GSTIN</th><th class="text-center">Document Date</th><th class="text-center">Eway Bill Date</th><th class="text-center">Taxable Amt</th><th class="text-center">Total Tax</th><th class="text-center">Total Amt</th>
							</tr>
						</thead>
	...
	<script src="${contextPath}/static/mastergst/js/reports/ewaybill_report.js" type="text/javascript"></script>
	<script type="text/javascript">
		...
		function generateData() {
			...
			var booksOrReturns = '';
			var type='';
			clearInvFiltersReportss('EWAYBILL');
			if(abc == 'Monthly'){
				$('#${varRetTypeCode}SummaryTable').css("display", "none");
				...
				var type='';
				//loadReportsUsersByClient('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', loadReportsUsersInDropDown);
				loadReportsInvoiceSupport('${client.id}', '${varRetType}', '${varRetTypeCode}', 0, parseInt(year[1]), type, loadReportsCustomersInDropdown);
				loadReportsInvTable('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, parseInt(year[1]), '${usertype}', '${fullname}', booksOrReturns, 'Yearly');
				loadReportsSummary('${id}','${varRetTypeCode}','${client.id}', 0, parseInt(year[1]), type);
				initiateCallBacksForMultiSelectReports('${varRetType}','${varRetTypeCode}');
				initializeRemoveAppliedFiltersReports('${varRetType}','${varRetTypeCode}');
				invsDownloads('${id}', '${client.id}', '${varRetType}','${varRetTypeCode}', 0, parseInt(year[1]), '${usertype}', '${fullname}', booksOrReturns, 'Yearly');
