...
<style>#adminusertable_filter input{width:150px;}
#adminusertable thead tr:nth-child(1){color: white;background-color: #5769bb;}
#adminusertable thead tr:nth-child(2){background-color: white;}
</style>
</head>
...
<body class="body-cls">
	...
	<div class="container">
		<div class="col-sm-12">&nbsp;</div>
			<div class="customtable db-ca-view ">
				<table id='adminusertable' class="row-border dataTable"
	...
	var table;
		...
		$(document).ready(function() {
			$('#adminusertable thead tr').clone(true).appendTo( '#adminusertable thead' );
			$('#adminusertable thead tr:eq(1) th').each( function (i) {
		        var title = $(this).text();
		  	...
		  	var table = $('#adminusertable').DataTable( {
			   "ajax": {
					url: '${contextPath}/activeUsersReportData/?id=<c:out value="${id}"/>&fullname=<c:out value="${fullname}"/>&usertype=<c:out value="${usertype}"/>',
			        type: 'GET'
			     },
		        orderCellsTop: true,
		        fixedHeader: true,
