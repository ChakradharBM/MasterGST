		...
		<div class="container" style="padding-top: 30px;">
			<div class="row">
				...
				<div class="customtable tabtable col-sm-12">
					<table id="dbTable_EXPIRED_IN_NEXT_45" class="display dataTable meterialform col-sm-12 p-0"  cellspacing="0" width="100%">
						<thead>
							<tr>
							<!-- <th><div class="checkbox"><label><input type="checkbox" id="datatableheadercheckbox" onClick="sendEmailAllUsers(this)" class="datatableheadercheckbox"/><i class="helper"></i></label></div></th> -->
								<th>Name</th>
								<th>Email</th>
								<th>Phone</th>
	...
	<div class="modal-dialog modal-md modal-right" role="document" style="height: 100%;">
    	<div class="modal-content" id="idSendMessageBody" style="height: 100%;">
        	<div class="modal-body">
				...
				<div class="form-group ">
  				...
  				<div class="form-group successmsg d-none">
    				<h6><i class="fa fa-check" style="font-size:32px;color:green"></i><span id="successmsg" class="text-success"></span></h6>
  				</div>
  				</div>
						...
						<div class="col-md-10" id="divcommentsdate">              				
	    					<select id="emailSubject" class="form-control">
	  							...
	  							<optgroup label="ASP DEVELOPERS">
								    <option value="MasterGST  API Subscription Expiring Soon">MasterGST  API Subscription Expiring Soon</option>
									<option value="MasterGST API Renewal - Reminder">MasterGST API Renewal - Reminder</option>
							  	</optgroup>
							  	<optgroup label="NON ASP DEVELOPERS">
								    <option value="MasterGST - Your Subscription Expiring Soon">Your Subscription Expiring Soon</option>
									<option value="MasterGST - Your Subscription Renewal - Reminder">Your Subscription Renewal - Reminder</option>
							  	</optgroup>
							</select>
						</div>
					</div>
			...
			<div class="modal-footer">			  
				<button type="button" class="btn  btn-blue-dark save-btn" id="mail_send_btn" onclick="sendMessagesData()">Send</button>
...
var beforeExpiryTable;
	$(document).ready(function() {
		$('#reports_lnk').addClass('active');
		
		
		loadExpiryUsersDetails('<c:out value="${id}"/>', 'EXPIRED_IN_NEXT_45');
		
	});
	
	function formatReferenceIdAndName(dat, type, row){
		var refid = dat.refid;
		var userfullname = dat.fullname;
		var email=dat.email;
		var epirydate=dat.subscriptionExpiryDate;
		return '<div class="button-type nottoedit"><button onClick="sendEmail(\''+refid+'\',\''+userfullname+'\',\''+email+'\',\''+epirydate+'\')" class="datatablebody btn btn-success" data-toggle="modal" data-target="#sendMessageModal" role="button" style="padding: 4px 10px 4px 10px; font-size: 14px;">SEND</button></div>';
	} 
...
$(document).ready(function(){
	...
	$('#emailSubject').on('change', function(){
		...
		$('#emailMeassage').val('Thank you being part of MasterGST, we have been noticed your subscription is getting Expired on Dated '+expirydate+'. This is to inform you that please do renewal before the date to avoid services interruption');	
	});
});
</script>
</body>
