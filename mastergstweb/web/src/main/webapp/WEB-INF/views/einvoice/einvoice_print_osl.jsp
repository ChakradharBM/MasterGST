...
<script src="${contextPath}/static/mastergst/js/client/currencyformat.js" type="text/javascript"></script>
<style>
      ...
      <div class="invoice-wrap" id="print-wrap">
        <div class="invoice-wrap-in pt-0 mt-0" style="height: 1000px;max-height: max-content;">
          <form class="meterialform invoiceform" style="min-height: 725px;height: auto;">
			...
			<c:if test='${not empty invoice.irnNo}'><strong class="mb-0"style="font-size:16px;">IRN : ${invoice.irnNo}</strong></c:if>
            <div class="clearfix bdr-btm">&nbsp;</div>
            <!-- invoice header start -->
            <div class="invoice-hdr userdetails">
              <div class="row m-0">
			<strong class="text-center" style="width:100%;float:left;font-size:16px;">${invText}</strong>
                <table width="100%" border="0" cellspacing="0" cellpadding="5">
                  <tr>
                    <td width="60%" style="vertical-align:top"><table width="100%" border="0" cellspacing="0" cellpadding="5" class="compdetails">
                        <tr>
                          <td valign="top">
						 <c:choose><c:when test="${(not empty invoice.billedtoname) && (not empty invoice.shipmentDtls && not empty invoice.shipmentDtls.addr1) && (not empty invoice.dispatcherDtls && not empty invoice.dispatcherDtls.addr1)}"></c:when><c:otherwise> <p class="m-1"><strong>GSTIN<span style="margin-left:47px">:</span></strong> ${client.gstnnumber}</p></c:otherwise></c:choose>
							<p class="m-1"><strong>PAN<span style="margin-left:56px">:</span> </strong>${client.pannumber}</p>
                            ...
                            <c:otherwise>
                            <c:if test="${empty invoice.branch}"><p class="m-1"><strong>Address<span style="margin-left:39px">:</span> </strong><c:choose><c:when test="${empty invoice.clientAddress}">${client.address}</c:when><c:otherwise>${invoice.clientAddress}</c:otherwise></c:choose></p></c:if>
                    ...
                    <td width="40%" style="vertical-align:top"><table width="100%" border="0" cellspacing="0" cellpadding="5">
                        ...
                        <tr>
                         ...
                         <tr>
                          ...
                          <td>:</td>
                          <td style="text-align: right;"><span class="f-14" id="invoicedate">${invoice.statename}</span></td>
                        </tr>
                         <tr>
                          <td><div class="lable-txt text-right">State Code</div></td>
                          <td>:</td>
                          <td style="text-align: right;"><span class="f-14" id="invoicedate">${istate_code}</span></td>
                        </tr>
			...
			<label>Reference : </label><span>${invoice.referenceNumber}</span>
			</c:if>
...
<%--                 <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
					    <td width="50%"class="comp-addres"><h3><strong>Details Of Receiver(Billed To) </strong></h3>
		                    ...
		                    <p style="margin-bottom: 3px;">${custAddress}</p>  
					   </td>
					   <td <c:if test="${not empty invoice.shipmentDtls && not empty invoice.shipmentDtls.addr1}">width="5%"</c:if> <c:if test="${empty invoice.shipmentDtls}">width="20%"</c:if>>&nbsp;</td>
					<c:if test="${not empty invoice.shipmentDtls && not empty invoice.shipmentDtls.addr1}">
                    <td width="47%"  class="comp-addres" style="vertical-align:top"><h3><strong>Details Of Consignee(Shipped To) </strong></h3>
                      <p>${invoice.shipmentDtls.trdNm}</p>
					  <p>${invoice.shipmentDtls.addr1}</p></td>
					</c:if>
                  </tr>
                </table> --%>
                <c:choose>
                  ...
                  <c:set var="aAmt" value="${item.rateperitem}" />
                    <tr>
                      ...
                      <td style="font-size:12px;" align="center"><c:choose><c:when test="${fn:contains(item.hsn, ' :')}">${fn:substring(item.hsn, 0, fn:indexOf(item.hsn, " :"))}</c:when><c:otherwise>${item.hsn}</c:otherwise></c:choose></td>
                       <c:if test="${empty pconfig || pconfig.enableQuantity eq true}"><td align="center" style="font-size:12px;"><span class="digitindformat">${item.quantity}</span></td></c:if>
                      <c:if test="${empty pconfig || pconfig.enableRate eq true}"><td class="textright" style="font-size:12px;"><span class="digitindformat">${item.rateperitem}</span></td></c:if>
                ...
                <table id="invoicetable1" class="display dataTable meterialform" cellspacing="0" width="100%" style="font-size:12px;">
                 ...
                 <c:if test="${not empty pconfig && pconfig.enableRoundOffAmt eq true}">
                     <tr>
					  ...
					  <tr>
					    <td style="font-size:12px;"><div align="right"><strong>Rounded Total</strong></div></td>
                  ...
                  <tr>
                    <td width="50%"><table width="100%" border="0" cellspacing="0" cellpadding="1">
					  ...
					  <tr><td><strong>Bank Details :</strong></td></tr>
						...
						<tr class="mt-1">
                          <td width=""><strong>Notes</strong></td>
                          <td width="">:</td>
						...
						<tr class="mt-1">
                          <td width=""><strong>Terms & Conditions</strong></td>
                          <td width="">:</td>
		...
		<c:if test="${empty client.enableAuthorisedSignatory || not client.enableAuthorisedSignatory}">
			<p style="text-align: center;">This is a computer generated invoice and does not need signature.</p>
		</c:if>
...
OSRECD.CurrencyFormatter.formatAll({
			selector: '.digitindformat'
		});
