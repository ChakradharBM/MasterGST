...
<div class="breadcrumbwrap">
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">GST Group Wise Sales Reports</li>
					</ol>
	...
	<div class="db-ca-wrap monthely1">
		<div class="container">
			...
			<h2>
				<span class="reports-monthly">Group wise Monthly Sales Report</span>
				<span class="reports-yearly" style="display: none;">Group wise Yearly Sales Report</span>
				<span class="reports-custom" style="display: none;">Group wise Custom Sales Report of <c:out value="${user.fullname}"/><c:choose><c:when test='${fn:length(client.fullname) > 25}'>${fn:substring(client.fullname, 0, 25)}..</c:when><c:otherwise>${client.fullname}</c:otherwise></c:choose></span>
			</h2>
			<p>
				<span class="reports-monthly">Group wise Monthly Sales Report gives you a summary of your monthly sales.<span id="monthly_clients_errormsg"></span></span>
				<span class="reports-yearly" style="display: none;">Group wise Yearly Sales Report gives you a summary of your monthly sales.<span id="yearly_clients_errormsg"></span></span>
				<span class="reports-custom" style="display: none;">Group wise Custom Sales Report gives you a summary of your monthly sales.<span id="custom_clients_errormsg"></span></span>
			</p>
...
var booksOrReturns = 'books';
	...
	<c:forEach items="${branches}" var="branch">
		$("#multiselectGSTR14").append($("<option></option>").attr("value","${branch.name}").text("${branch.name}"));
	</c:forEach>
	multiselrefresh_reports('#multiselectGSTR14', '- Branches -' , '${varRetType}');
	<c:forEach items="${verticals}" var="vertical">
		$("#multiselectGSTR15").append($("<option></option>").attr("value","${vertical.name}").text("${vertical.name}"));
	</c:forEach>
	multiselrefresh_reports('#multiselectGSTR15', '- Verticals -' , '${varRetType}');
	...
	$('span#filingoption').css("vertical-align", "bottom");		
		...
		if(selectGroupArrayClientids.length == 0 || selectGroupArrayClientids[0] == 'GROUPS_NOTFOUND'){
			...
			if(clientidsArray.length > 0 && clientidsArray[0] != 'CLIENTS_NOTFOUND'){
				...
				initiateCallBacksForMultiSelectGlobalReports('${varRetType}','${varRetTypeCode}');
				initializeRemoveAppliedFiltersGlobalReports('${varRetType}','${varRetTypeCode}');
				invsDownloads('${id}', selectGroupArrayClientids, '${varRetType}','${varRetTypeCode}', fromtime, totime, '${usertype}', '${fullname}', booksOrReturns, abc);
