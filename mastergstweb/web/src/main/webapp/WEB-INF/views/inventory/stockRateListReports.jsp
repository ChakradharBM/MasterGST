...
<meta charset="utf-8" />
    ...
    <title>MasterGST | Rate List Report</title>
...
<style>
#reports_stockRate tbody tr td:first-child{color:black;}
</style>
<body class="body-cls">
	     	...
	     	<div class="row">
	        	<div class="col-md-12 col-sm-12">
	            	<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">Rate List Report</li>
	                </ol>
 ...
 <div class="db-ca-wrap">
		<div class="container">
			<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right mb-3" role="button" style="padding: 4px 25px;">Back</a>
		        <h4>Rate List Report Of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></h4>
		        <div class="">
		         <div class="customtable db-ca-view reportsdbTableStockRate">
					<table id='reports_stockRate' class="row-border dataTable meterialform" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th class="text-center">Item Code</th><th class="text-center">Item Name</th><th class="text-center">MRP Price</th><th class="text-center">Selling Price</th>
							</tr>
						</thead>
						<tbody id='stockRate_body'></tbody>
					</table>
				</div>
				</div>
	  </div>
...
loadStockSummaryTable('${id}', '${client.id}', '${month}', '${year}', '${usertype}', '${fullname}','Rate');
stockSummaryExcelDownloads('${id}', '${client.id}', '${month}', '${year}', '${usertype}', '${fullname}',' ', 'Rate', ' ');
</script>
</body>
</html>