...
<meta charset="utf-8" />
    ...
    <title>MasterGST | Customer wise Item Report</title>
...
#reports_stockItm tbody tr td:first-child{color:black;}
.sitm::after,#sitm_filing_option1::after{display:none;}
.datetime-wrap{display: inline-flex};
.dataTables_length select {background-color: white;}
span.select2.select2-container.select2-container--default.select2-container--below.select2-container--open , span.select2.select2-container.select2-container--default.select2-container--focus , span.select2.select2-container.select2-container--default{width:180px!important}
...
<%@include file="/WEB-INF/views/includes/client_header.jsp"%>
	     	...
	     	<div class="row">
	        	<div class="col-md-12 col-sm-12">
	            	<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">Customer wise Item Report</li>
	                </ol>
	...
	<div class="db-ca-wrap">
		...
		<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right mb-3" role="button" style="padding: 4px 25px;">Back</a>
			<h4>
				<span class="reports-monthly">Monthly Customer wise Item Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-yearly" style="display: none;">Yearly Customer wise Item Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-custom" style="display: none;">Custom Customer wise Item Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
			</h4>
			<p>
				<span class="reports-monthly">Monthly Customer wise Item Report gives you a summary of your monthly Stock usage.</span>
				<span class="reports-yearly" style="display: none;">Yearly Customer wise Item Report gives you a summary of your monthly Stock usage.</span>
				<span class="reports-custom" style="display: none;">Custom Customer wise Item Report gives you a summary of your monthly Stock usage.</span>
			</p>
			<label>Customer Name:</label>
			<select id="stock_customername" class="custdrop_search" name="state" style="width:180px!important">
				<option value=""> - Select Customer - </option>
				<c:forEach items="${customers}" var="customer">
					<option value="<c:out value="${customer.name}"/>"><c:out value="${customer.name}"/></option>
				</c:forEach>
			</select>
			<span id="custname_error" style="color:red;"></span>
			
				<div class="dropdown chooseteam mr-0" style="z-index:2;">
					<span class="dropdown-toggle yearly sitm" data-toggle="dropdown" id="itm_fillingoption" style="margin-right: 10px; display: inline-flex;"><label>Report Type:</label>
					...
					<span style="display:none" class="yearly-sp"> 
						<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="sitm_filing_option1"	style="margin-right: 10px; display: inline-flex;">
						...
						<div class="dropdown-menu ret-type1" id="sitm_financialYear1" style="width: 108px !important; min-width: 36px; left: 61%; top: 26px; border-radius: 2px">
			...
			<div class="customtable db-ca-view salestable reportsdbTableStockItm">
					<table id='reports_stockItm' class="row-border dataTable meterialform" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th class="text-center">Customer Name</th><th class="text-center">Item Name</th><th class="text-center">Sales Quantity</th><th class="text-center">Sales Amount</th><th class="text-center">Purchase Quantity</th><th class="text-center">Purchase Amount</th>
							</tr>
						</thead>
						<tbody id='stockItm_body'></tbody>
					</table>
				</div>
		</div>
	</div>
	...
	$(function(){
		...
		loadPartyWiseTable('${id}', '${client.id}', '${month}', '${year}', '${usertype}', '${fullname}','Monthly', '');
	});
	function generateData() {
		var abc = $('#sitm_fillingoption span').html();
		var custName = $('#stock_custname').val();
/* 		if(itemName == ""){
			...
			if(abc == 'Monthly'){
				...
				var type='';
				loadPartyWiseTable('${id}', '${client.id}', mn, yr, '${usertype}', '${fullname}', 'Monthly', custName);
			}else if (abc == 'Yearly') {
				...
				var type='';
				loadPartyWiseTable('${id}', '${client.id}', 0, year[1], '${usertype}', '${fullname}', 'Yearly', custName);
			}else{
				...
				var type ='custom';
				loadPartyWiseTable('${id}', '${client.id}', fromtime, totime, '${usertype}', '${fullname}', type, custName);
			}
		//}
	}
	</script>
