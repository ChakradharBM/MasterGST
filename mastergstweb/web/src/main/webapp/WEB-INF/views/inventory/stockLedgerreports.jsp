...
<meta charset="utf-8" />
    ...
    <title>MasterGST | Stock Ledger Report</title>
...
.sledger::after,#sledger_filing_option1::after{display:none;}
...
<%@include file="/WEB-INF/views/includes/client_header.jsp"%>
	...
	<div class="db-ca-wrap">
		...
		<a href="${contextPath}/dreports/${id}/${fullname}/${usertype}/${client.id}/${month}/${year}" class="btn btn-blue-dark pull-right mb-3" role="button" style="padding: 4px 25px;">Back</a>
			<h4>
				<span class="reports-monthly">Monthly Stock Ledger Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-yearly" style="display: none;">Yearly Stock Ledger Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
				<span class="reports-custom" style="display: none;">Custom Stock Ledger Report of <c:choose><c:when test='${fn:length(client.businessname) > 45}'>${fn:substring(client.businessname, 0, 45)}..</c:when><c:otherwise>${client.businessname}</c:otherwise></c:choose></span>
			</h4>
			<p>
				<span class="reports-monthly">Monthly Stock Ledger Report gives you a summary of your monthly Stock Ledger.</span>
				<span class="reports-yearly" style="display: none;">Yearly Stock Ledger Report gives you a summary of your monthly Stock Ledger.</span>
				<span class="reports-custom" style="display: none;">Custom Stock Ledger Report gives you a summary of your monthly Stock Ledger.</span>
			</p>
			<label>Item Name:</label>
			<select id="stock_itemname" class="itemdrop_search" name="state" style="width:180px!important">
			...
			<span id="itemname_error" style="color:red;"></span>
				...
				<div class="dropdown chooseteam mr-0" style="z-index:2;">
					<span class="dropdown-toggle yearly sledger" data-toggle="dropdown" id="sledger_fillingoption" style="margin-right: 10px; display: inline-flex;"><label>Report Type:</label>
					...
					<span style="display:none" class="yearly-sp"> 
						<span class="dropdown-toggle yearly" data-toggle="dropdown"	id="sledger_filing_option1"	style="margin-right: 10px; display: inline-flex;">
			...
			<div class="customtable db-ca-view salestable reportsdbTableStockLedger">
					<table id='reports_stockLedger' class="row-border dataTable meterialform" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th class="text-center">Item</th><th class="text-center">Date</th><th class="text-center">Voucher</th><th class="text-center">Inwards Quantity</th><th class="text-center">Outwards Quantity</th><th class="text-center">Price/Rate</th><th class="text-center">Balance Quantity</th>
							</tr>
						</thead>
						<tbody id='stockLedger_body'></tbody>
					</table>
				</div>
		</div>
	</div>
	...
	$('#stockLedger_body').append('<tr><td colspan="7" style="text-align: center;height: 25px;vertical-align: inherit;border-right: 1px solid lightgray !important;">No Records Found</td></tr>');
	$(function(){
		var date = new Date();
		...
		//loadStockLedgerTable('${id}', '${client.id}', '${month}', '${year}', '${usertype}', '${fullname}','Monthly', '');
	});
	function generateData() {
		var abc = $('#sledger_fillingoption span').html();
		var itemName = $('#stock_itemname').val();
 		if(itemName == ""){
			...
			if(abc == 'Monthly'){
				...
				var type='';
				loadStockLedgerTable('${id}', '${client.id}', mn, yr, '${usertype}', '${fullname}', 'Monthly', itemName);
			}else if (abc == 'Yearly') {
				...
				var type='';
				loadStockLedgerTable('${id}', '${client.id}', 0, year[1], '${usertype}', '${fullname}', 'Yearly', itemName);
			}else{
				...
				var type ='custom';
				loadStockLedgerTable('${id}', '${client.id}', fromtime, totime, '${usertype}', '${fullname}', type, itemName);
			}
		}
	}
	</script>
