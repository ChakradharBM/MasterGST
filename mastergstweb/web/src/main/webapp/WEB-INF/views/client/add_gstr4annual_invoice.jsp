...
<script src="${contextPath}/static/mastergst/js/client/gstr4annual.js" type="text/javascript"></script>
...
<script type="text/javascript">
	...
	$(function () {
		indexObj['tab6'] = parseInt("<c:choose><c:when test='${not empty gstr4annualinv && not empty gstr4annualinv.outsupply}'>${fn:length(gstr4annualinv.outsupply)}</c:when><c:otherwise>1</c:otherwise></c:choose>");
		...
		if('<c:out value="${gstr4annualcmp}"/>' != "" && '<c:out value="${gstr4annualcmp}"/>' != null){
			...
			var gstr4Tab7Content = '';
			$('#tab7_tdstcsBody').append(gstr4Tab7Content)
		}
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
					...
					<span class="dropdown chooseteam">
					  <span id="fillingoption"><b>Filing Option:</b> <span id="filing_option">Yearly</span></span>
					 </span>
	...
	<div class="db-ca-wrap db-ca-gst-wrap">
		<div class="container" style="min-height: 400px">
			<div class="row">
				...
				<form:form method="POST" id="supForm" data-toggle="validator" class="meterialform gstr4annualinvoiceform" name="gstr4annualinvoceform" action="${contextPath}/saveAnnualinvoice/${returntype}/${usertype}/${month}/${year}" modelAttribute="invoice">
					<div class="col-md-12 col-sm-12">
						...
						<div id="accordion" class="inneracco">
						  ...
						  <!-- Table 3 start -->
						   <div class="card">
						    ...
						    <div class="group upload-btn">
								<span class="pull-right"> <a class="btn btn-sm btn-blue-dark tptwo-edit" onClick="clickinwordOrOutwardtab6Edit('.tptwo-save', '.tptwo-cancel', '.tptwo-edit','.tptwo-input', '.addmorewrap');">Edit</a> <a class="btn btn-sm  btn-blue-dark tptwo-cancel" onClick="clickSave('.tptwo-save', '.tptwo-cancel', '.tptwo-edit','.tptwo-input', '.addmorewrap');" >Save</a> <a class="btn btn-sm  btn-blue-dark tptwo-cancel" onClick="clickCancel('.tptwo-save', '.tptwo-cancel', '.tptwo-edit','.tptwo-input', '.addmorewrap',2);" >Cancel</a></span>
							</div>
						    <div id="collapseinnerThree" class="collapse show" aria-labelledby="headinginnerTwo" data-parent="#accordion">
						      <div class="card-body p-2">
						       <div class="customtable db-ca-gst tabtable1 mt-2">
							<table id="inwordOrOutwardTable6" class="display row-border dataTable meterialform" cellspacing="0" width="100%">
								<thead>
									<tr>
										<th class="text-left" rowspan="2">SR.No</th>
								...
								<tbody id="inwordOrOutwardtab6Body">
									<tr id="inwordOrOutwardtab6Bdytr1">
										<td id="sno_row1" class="text-left">1</td>
										<td class="text-right form-group gst6-error"><select class="form-control" id="typeOfSupply1" name="gstr4annualinv.outsupply[0].type" onchange="typeOfSupply('1')"><option value=""> select </option><option value="Inward">Inward</option><option value="Outward">Outward</option></select></td>
										<td class="text-right form-group gst6-error"><input id="tab6amtval1" type="text"  name="gstr4annualinv.outsupply[0].txval" class="form-control" onkeyup="findTaxableamt('1')"/></td>
										<td class="text-left form-group gst6-error"><select name="gstr4annualinv.outsupply[0].txval" class="form-control" id="typeOfSupplyTaxRate1" onchange="typeOfSupplyTaxRate('1')"><option value=""> select </option></select></td>
										<td class="text-right form-group gst6-error"><input name="gstr4annualinv.outsupply[0].rate" data-variavel="tab6Afield1" id="tab6Iamt1" type="text" class="form-control tpthree-input" placeholder="0.00"/></td>
										<td class="text-right form-group gst6-error"><input id="tab6Camt1" type="text" class="form-control tpthree-input" placeholder="0.00"/></td>
										<td class="text-right form-group gst6-error"><input id="tab6Samt1" type="text" class="form-control tpthree-input" placeholder="0.00"/></td>
										<td class="text-right form-group gst6-error"><input id="tab6Cess1" type="text" class="form-control tpthree-input" placeholder="0.00"/></td>
										<td class="text-right form-group gst6-error">
											<a class="btn btn-blue-dark btn-sm pull-right addmore mt-1 d-none" id="addmore1" onclick="addInwardOrOutwardRecord()">Add</a>
										</td>
									</tr>									
								</tbody>
								<tfoot>
									<tr>
										...
										<th></th>
										<th class="pl-1"><input type="text" class="form-control tpthree-input" id="totIamt" placeholder="0.00"/></th>
										<th class="pl-1"><input type="text" class="form-control tpthree-input" id="totCamt" placeholder="0.00"/></th>
										<th class="pl-1"><input type="text" class="form-control tpthree-input" id="totSamt" placeholder="0.00"/></th>
										<th class="pl-1"><input type="text" class="form-control tpthree-input" id="totCess" placeholder="0.00"/></th>
										<th class="pl-1"></th>
									</tr>
								</tfoot>
							</table>
						  ...
						  <!-- Table 3 End -->
						   ...
						   <div class="card">
							...
							<table id="dbTable2" class="display row-border dataTable meterialform" cellspacing="0" width="100%">
								...
								<tbody id="tab7_tdstcsBody">
									<c:forEach items="${gstr4annualcmp.tdstcs}" var="tdstcs" varStatus="loop">
										<tr id="tdstcs${loop.index}">
											<td class="text-left form-group"><input type="text" id="tab7_tdstcs${loop.index}Ctin" class="form-control" value="${tdstcs.ctin}"/></td>
											<td class="text-left form-group"><input type="text" id="tab7_tdstcs${loop.index}Gross"class="form-control tpthree-input" placeholder="0.00" value="${tdstcs.gross_val}"/></td>
											<td class="text-left form-group"><input type="text" id="tab7_tdstcs${loop.index}Camt" class="form-control tpthree-input" placeholder="0.00" value="${tdstcs.camt}"/></td>
											<td class="text-left form-group"><input type="text" id="tab7_tdstcs${loop.index}Samt" class="form-control tpthree-input" placeholder="0.00" value="${tdstcs.samt}"/></td>
										</tr>
									</c:forEach>
									<!-- <tr>
	        ...
	        <div class="row gstr-info-tabs pl-2 pr-2 pt-1">
					...
					<div class="tab-content col-md-12 mb-3 mt-1">
									...
									<div class="customtable">
										<table id="gstintable" class="display row-border dataTable meterialform">
											<thead>
												...
												<th rowspan="2">Trade Name</th><th rowspan="2">POS</th><th rowspan="2">Taxable Value</th>
												<th colspan="4" class="text-center">Amount Of Tax</th><th rowspan="2">Details</th></tr>
			        		  ...
			        		  <div class="tab-pane col-md-12" id="amountstab" role="tabpane2">
			        		  	<div class="customtable">
									<table id="amountswisetable" class="display row-border dataTable meterialform">
										<thead>
											<tr><th>SR.No</th>
											<th>Tax Rate</th><th>Taxable Value</th>
											<th>IGST</th><th>CGST</th><th>SGST</th><th>CESS</th>
										</thead>
										<tbody id="amountswisetableData"></tbody>
									</table>
								</div>
			        		  </div>
			        </div>
...
function addmorerow(a){
	...
	function autocalculategstr4(){
		var yr = $('#gstr4FinancialYear').val();
		otpExpiryCheck();
		window.location.href = '${contextPath}/getpopulategstr4annualdata'+commonSuffix+'/'+clientId+'/03/'+yr;
		if(otpExpirycheck == 'OTP_VERIFIED'){
			window.location.href = '${contextPath}/getpopulategstr4annualdata'+commonSuffix+'/'+clientId+'/03/'+yr;
		}else{
			errorNotification('Your OTP Session Expired. Click <a href="#" class="btn btn-sm btn-blue-dark" onclick="invokeOTP(this)">Verify Now</a> to proceed further.');
