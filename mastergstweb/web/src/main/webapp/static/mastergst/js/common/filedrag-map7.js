...
(function() {
	...
	function FileDragHover(e) {
		e.stopPropagation();
		e.preventDefault();
		$id("supplierFile").className = (e.type == "dragover" ? "hover" : ".xls");
...
//alert(files);
		...
		if(e.dataTransfer.files){
			...
			$("#supplierFile").prop("files", e.dataTransfer.files);
			$('#idSheet7').show();
	...
	function ParseFile(file) {
		...
		var x = document.getElementById('supplierFile').value;
		Output(
	...
	function Init() {
		...
		var fileselect = $id("supplierFile"),
			filedrag = $id("filedragi"),
			submitbutton = $id("submitbutton7");
