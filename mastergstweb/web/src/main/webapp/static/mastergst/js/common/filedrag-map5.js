...
(function() {
	...
	function FileDragHover(e) {
		e.stopPropagation();
		e.preventDefault();
		$id("itemFile").className = (e.type == "dragover" ? "hover" : ".xls");
...
//alert(files);
		...
		if(e.dataTransfer.files){
			...
			$("#itemFile").prop("files", e.dataTransfer.files);
			$('#idSheet5').show();
	...
	function ParseFile(file) {
		...
		var x = document.getElementById('itemFile').value;
		Output(
