...
// Created by the Bx team at OSRECD Technologies (https://osrec.co.uk)
//
...
OSRECD.CurrencyFormatter = {
	...
	getFormatDetails: function getFormatDetails(p) {
		var locales = OSRECD.CurrencyFormatter.locales;
		var defaultLocales = OSRECD.CurrencyFormatter.defaultLocales;
	...
	getFormatter: function getFormatter(p) {
		var formatDetails = OSRECD.CurrencyFormatter.getFormatDetails(p);
		...
		var format = function format(n, f) {
			var formattedNumber = OSRECD.CurrencyFormatter.toFixed(Math.abs(n), f.decimalPlaces);
	...
	formatAll: function formatAll(p) {
		var formatter = OSRECD.CurrencyFormatter.getFormatter(p);
		...
		for (var i = 0; i < matches.length; ++i) {
			try {
				...
				if (typeof formatters[ccy] === 'undefined') {
					formatters[ccy] = OSRECD.CurrencyFormatter.getFormatter({ currency: ccy });
				}
	...
	format: function format(n, p) {
		var formatterFunction = OSRECD.CurrencyFormatter.getFormatter(p);

		return formatterFunction(n);
	},

	parse: function parse(str, p) {
		var decimal = OSRECD.CurrencyFormatter.getFormatDetails(p).decimal;
...
if (hasDefine) {
	// AMD Module
	define([], function () {
		return OSRECD.CurrencyFormatter;
	});
} else if (hasExports) {
	// Node.js Module
	module.exports = OSRECD.CurrencyFormatter;
} else {
