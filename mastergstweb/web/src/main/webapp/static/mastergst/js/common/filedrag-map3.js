...
(function() {
	...
	function FileDragHover(e) {
		e.stopPropagation();
		e.preventDefault();
		$id("productFile").className = (e.type == "dragover" ? "hover" : ".xls");
...
//alert(files);
		...
		if(e.dataTransfer.files){
			...
			$("#productFile").prop("files", e.dataTransfer.files);
			$('#idSheet3').show();
	...
	function ParseFile(file) {
		...
		var x = document.getElementById('productFile').value;
		Output(
	...
	function Init() {
		...
		var fileselect = $id("productFile"),
			filedrag = $id("filedragp"),
