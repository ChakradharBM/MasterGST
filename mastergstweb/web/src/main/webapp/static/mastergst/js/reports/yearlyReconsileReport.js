...
function loadReportsUsersByClient(id, clientid, callback){
	...
	$.ajax({
		...
		success : function(response) {
			...
			callback(response);
			//loadReportsUsersInDropDown(response)
		}
	});
}

function loadReportsUsersInDropDown(response){
	...
	multiselrefresh_reports('#MMmultiselect3', '- User -');
	$('#MMmultiselect3').multiselect('rebuild');
}

function loadReportsInvoiceSupport(id, clientId, year, callback){
	var urlStr = _getContextPath()+'/getReconsileInvsSupport/'+clientId+'/4/'+year;
	$.ajax({
		url: urlStr,
		async: true,
		cache: false,
		...
		success : function(response) {
			callback(response);
			//loadReportsCustomersInDropdown(response)
		}
	});
}

function loadReportsCustomersInDropdown(response){
	...
	multiselrefresh_reports('#MMmultiselect4', customerType);
...
function initiateCallBacksForMultiSelectReports(){
	...
	for(i=1;i<=6;i++){
		...
		multiselrefresh_reports('#MMmultiselect'+i, multiSelDefaultVals[i]);
	}
}
function multiselrefresh_reports(idval, descVal){
	$(idval).multiselect({
...
function initializeRemoveAppliedFiltersReports(){
...
function applyInvFilterss() {
	...
	reloadInvTable(statusArr, typeArr, userArr, vendorArr, branchArr, verticalArr);
}
...
function clearMMFilters(){
	...
	$('#divMMFilters').html('');
	$('.normaltable .filter').css("display","none");
	reloadInvTable(new Array(), new Array(), new Array(), new Array(), new Array(), new Array(),);
}

function loadReportsInvTable(id, clientid, year){
	var pUrl =_getContextPath()+'/getInvs/'+clientid+'/4/'+year+'?returntype=Purchase Register';
	...
	invTableReports = $('#reconsileDbTable').DataTable({
		"dom": 'f<"toolbar">lrtip<"clear">',
	     ...
	     "ajax": {
	         ...
	         'dataSrc': function(resp){
	        	 ...
	        	 //resp.recordsTotal = resp.prInv.totalElements;
	        	 //resp.recordsFiltered = resp.prInv.totalElements;
	        	 //$('#updatefilter_summary_reports').show();
	        	 loadReportsTotals(resp.amts);
		...
		"searching": true,
		"order": [[4,'desc']],
		'columns': getInvColumns(clientId, year),
...
function getInvColumns(clientId, year){
	...
	var billtogtnn = {data: function ( data, type, row ) {
		...
		if(invStatus == 'Manual Matched'){
			...
			if(data.origin == "Purchase Register" && data.gstr2 != null){
				...
				return '<span onClick="showMismatchInv(\''+docid+'\',\''+invStatus+'\')"><div class="tdline_1 '+colorclass+'">'+gstnnum+'</div><div class="tdline_2 '+colorclass+'">'+g2ctin+'</div></span>';
			}else if(data.origin == "GSTR2A" && data.gstr2 != null){
				var g2ctin = '';
				if(data.gstr2.b2b && data.gstr2.b2b.length > 0){
					if(data.gstr2.b2b[0].ctin != undefined){
				...
				return '<span onClick="showMismatchInv(\''+docid+'\',\''+invStatus+'\')"><div class="tdline_1 color-red">-</div><div class="tdline_2 color-red">'+g2ctin+'</div></span>';
			}else{
				if(data.purchaseRegister.b2b && data.purchaseRegister.b2b.length > 0){
					if(data.purchaseRegister.b2b[0].ctin != undefined){
						gstnnum = data.purchaseRegister.b2b[0].ctin;
				...
				return '<span onClick="showMismatchInv(\''+docid+'\',\''+invStatus+'\')"><div class="tdline_1 color-red">'+gstnnum+'</div><div class="tdline_2 color-red">-</div></span>';
			}
		}		
	}};
	...
	var status = {data: function ( data, type, row ) {
		...
		if(invStatus == 'Manual Matched'){
			...
			return '<span class="text-left invoiceclk bluetxt" onClick="showMismatchInv(\''+docid+'\',\''+invStatus+'\')">'+invStatus+'</span>';
		}
	}};
	return [chkBx, info, itype, billtoname, fp, invsNo, invDate, billtogtnn, totamt, taxblamt, totlTax, status, notes];
}

function reloadInvTable(statusArr, typeArr, userArr, vendorArr, branchArr, verticalArr){
...
function loadReconcileSummary(id, clientId, year){
	var urlStr = _getContextPath()+'/getReconcileSummary/'+clientId+'/4/'+year;
	$.ajax({
		url: urlStr,
		async: true,
		cache: false,
...
function loadReportsTotals(totalsData){
...
function updateMismatchDataNew(btn, acceptFlag) {
	...
	$.ajax({
		type: "POST",
		url: _getContextPath()+"/prfmmismatch/"+acceptFlag,
		async: false,
		cache: false,
	...
	$.ajax({
		...
		success : function(response) {
			...
			if(response.monthlyoryearly != undefined){
				...
				$('.reconprogress,.prinfo').css("display","none");
			}
...
function updateMainMisMatchSelection(chkBox) {
	...
	if(chkBox.checked) {
		invTableReports.rows().every(function () {
			var row = this.data();
